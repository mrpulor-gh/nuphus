...
use crate::handler::{REQUEST_TIMEOUT, RuntimeExecutionMode};
...
impl Frame {
    ...
    pub fn set_execution_context(
        ...
        match dom_world {
            DOMWorldKind::Main => self.main_world.set_context_id(execution_context_id),
            DOMWorldKind::Secondary => self.secondary_world.set_context_id(execution_context_id),
        }
    }
...
impl FrameManager {
    ...
    pub fn init_commands(
        ...
        if runtime_execution_mode == RuntimeExecutionMode::Persistent {
            commands.push((
...
mod tests {
    ...
    fn initialization_methods(mode: RuntimeExecutionMode) -> Vec<String> {
        let mut commands = FrameManager::init_commands(Duration::from_secs(1), mode);
        let mut methods = Vec::new();
        loop {
            match commands.poll(Instant::now()) {
                Poll::Ready(Some(Ok((method, _)))) => {
                    methods.push(method.to_string());
                    assert!(commands.received_response(method.as_ref()));
                }
                Poll::Ready(None) => return methods,
    ...
    #[test]
    fn persistent_initialization_enables_runtime() {
        assert!(
            initialization_methods(RuntimeExecutionMode::Persistent)
                .iter()
                .any(|method| method == "Runtime.enable")
        );
    }

    #[test]
    fn on_demand_initialization_does_not_enable_runtime() {
        assert!(
            initialization_methods(RuntimeExecutionMode::OnDemand)
                .iter()
                .all(|method| method != "Runtime.enable")
        );
    }

    #[test]
    fn navigation_invalidates_cached_contexts() {
        let old_context = ExecutionContextId::new(42);
        let frame_payload: CdpFrame = serde_json::from_value(serde_json::json!({
            "id": "main",
            "loaderId": "loader-2",
        ...
        .expect("valid CDP frame fixture");

        let mut manager = FrameManager::new(Duration::from_secs(1));
        let mut frame = Frame::new(FrameId::new("main"));
        frame.set_execution_context(DOMWorldKind::Main, old_context);
        manager.main_frame = Some(frame.id().clone());
        manager.frames.insert(frame.id().clone(), frame);

        manager.on_frame_navigated(&frame_payload);

        assert_eq!(manager.main_frame().unwrap().execution_context(), None);
    }
}
