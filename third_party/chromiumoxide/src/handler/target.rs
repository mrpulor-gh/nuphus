...
use crate::handler::{PageInner, REQUEST_TIMEOUT, RuntimeExecutionMode};
        ...
        if self.page.is_none() {
            if let Some(session) = self.session_id.clone() {
                let handle = PageHandle::new(
                    ...
                    self.config.runtime_execution_mode,
                );
        ...
        match &mut self.init_state {
            ...
            TargetInit::InitializingFrame(cmds) => {
                ...
                if let Poll::Ready(poll) = cmds.poll(now) {
                    return match poll {
                        None => {
                            let isolated_world_cmds = if self.config.runtime_execution_mode
                                == RuntimeExecutionMode::Persistent
                            {
                                self.frame_manager.ensure_isolated_world(UTILITY_WORLD_NAME)
                            } else {
                                None
                            };
                            if let Some(isolated_world_cmds) = isolated_world_cmds {
            ...
            if let Some(handle) = self.page.as_mut() {
                while let Poll::Ready(Some(msg)) = Pin::new(&mut handle.rx).poll_next(cx) {
                    match msg {
                        ...
                        TargetMessage::CacheExecutionContext(ctx) => {
                            if let Some(frame) = self.frame_manager.frame_mut(&ctx.frame_id) {
                                frame
                                    .set_execution_context(ctx.dom_world, ctx.execution_context_id);
                            }
                        }
...
pub struct TargetConfig {
    ...
    pub runtime_execution_mode: RuntimeExecutionMode,
}
...
impl Default for TargetConfig {
    fn default() -> Self {
        Self {
            ...
            cache_enabled: true,
            runtime_execution_mode: RuntimeExecutionMode::Persistent,
        }
    }
}
...
pub enum TargetMessage {
    ...
    /// Cache a context discovered without enabling the Runtime domain.
