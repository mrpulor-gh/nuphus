...
// src/main-window/workflow-canvas/types.ts
var SYNTH = {
  entry: (layerId) => `${layerId}::entry`,
  cond: (stepId) => `${stepId}::cond`,
  external: (layerId, varName) => `${layerId}::external::${varName}`,
  loopback: (stepId) => `${stepId}::loopback`
};

// src/main-window/workflow-canvas/dataEdges.ts
var VAR_REF_RE = /\{\{\s*([A-Za-z_]\w*)((?:[^}]*)?)\}\}/g;
function isExternalVar(name) {
  return name === "params" || name === "ENV";
}
function parsePipes(rest) {
  if (!rest) return [];
  return rest.split("|").map((p) => p.trim()).filter(Boolean).map((p) => {
    const m = p.match(/^([A-Za-z_]\w*)/);
    return m ? m[1] : "";
  }).filter(Boolean);
}
function scanTemplateRefs(text, out) {
...
function scanJsonRefs(v, out) {
  if (typeof v === "string") {
    scanTemplateRefs(v, out);
  } else if (Array.isArray(v)) {
    for (const item of v) scanJsonRefs(item, out);
  } else if (v && typeof v === "object") {
    for (const val of Object.values(v)) scanJsonRefs(val, out);
  }
}
function scanVarRef(r, out) {
  ...
  if (typeof r === "object" && "var" in r && typeof r.var === "string") {
    const root2 = r.var.split(".")[0];
    if (root2 && !isExternalVar(root2) && !root2.startsWith("ENV:")) {
      out.push({ varName: root2, pipes: [] });
    }
  }
}
function scanCondition(cond, out) {
  if (!cond || typeof cond !== "object") return;
  const c = cond;
  for (const key of ["equals", "not_equals", "contains", "starts_with", "regex", "gt", "lt", "gte", "lte"]) {
    const arr = c[key];
    if (Array.isArray(arr)) for (const r of arr) scanVarRef(r, out);
  }
  for (const key of ["not_empty", "empty"]) {
    if (key in c) scanVarRef(c[key], out);
  }
}
var CONTAINER_KEYS = /* @__PURE__ */ new Set(["seq", "do", "then", "else", "auto"]);
function scanActionPayload(value, out) {
  ...
  const obj = value;
  const ifDef = obj.if;
  if (ifDef) scanCondition(ifDef.condition, out);
  const loopDef = obj.loop;
  if (loopDef) {
    if (loopDef.until) scanCondition(loopDef.until, out);
    const forEach = loopDef.for_each;
    if (forEach?.items) scanVarRef(forEach.items, out);
  }
  const assertDef = obj.assert;
  if (assertDef) scanCondition(assertDef.condition, out);
...
function profileStep(step) {
  ...
  return {
    stepId: step.id,
    produces: typeof step.capture === "string" && step.capture ? step.capture : void 0,
    consumes
  };
}
function walkSteps(steps, visit, parentId = null) {
  for (const step of steps) {
    visit(step, parentId);
    const d = step.do;
    if (!d || typeof d !== "object") continue;
    if (Array.isArray(d.seq)) walkSteps(d.seq, visit, step.id);
    const loop = d.loop;
    ...
    if (Array.isArray(d.auto)) walkSteps(d.auto, visit, step.id);
  }
}

// src/main-window/workflow-canvas/projection.ts
var KNOWN_ACTION_KEYS = [
  "tool",
  "seq",
  "loop",
  "if",
...
function containerLanes(step) {
  ...
  if (Array.isArray(d.auto) && d.auto.length > 0)
    return { kind: "wait", lanes: [{ id: "main", title: "\u81EA\u52A8\u64CD\u4F5C" }] };
  return null;
}
function laneSteps(step, lane) {
  const d = step.do;
  if (!d || typeof d !== "object") return [];
  if (Array.isArray(d.seq) && lane === "main") return d.seq;
  const loop = d.loop;
  ...
  if (Array.isArray(d.auto) && lane === "main") return d.auto;
  return [];
}
function childCount(step) {
  const c = containerLanes(step);
  if (!c) return 0;
  return c.lanes.reduce((n, l) => n + laneSteps(step, l.id).length, 0);
}
function varRefText(r) {
  if (r === void 0 || r === null) return "";
...
function conditionSummary(cond) {
  ...
  for (const [key, sym] of ops) {
    ...
    const v = c[key];
    if (Array.isArray(v))
      return `${varRefText(v[0])} ${sym} ${varRefText(v[1])}`.trim();
    return `${varRefText(v)} ${sym}`.trim();
  }
  return "";
}
function containerSummary(step, kind) {
  const d = step.do;
  if (!d) return void 0;
  if (kind === "loop") {
    const def = d.loop;
  ...
  if (kind === "if") {
    ...
    return s ? `\u6761\u4EF6 ${s}` : void 0;
  }
  if (kind === "wait") {
    return typeof d.wait === "string" ? `\u7B49\u5F85 ${d.wait}` : void 0;
  }
  return void 0;
}
function onErrorLabel(step) {
  const oe2 = step.on_error;
  if (!oe2 || oe2 === "abort") return void 0;
  if (typeof oe2 === "string") return oe2;
  if (typeof oe2 === "object") {
    if ("retry" in oe2) return "retry";
    if ("allow_codes" in oe2) return "allow_codes";
  }
  return void 0;
}
function buildTreeProfile(rootSteps) {
  const order = [];
...
function nearestProducer(tree, varName, consumerId) {
...
function loopScopeOf(step) {
  const d = step.do;
  const loop = d?.loop;
  if (!loop) return null;
  return { loopStepId: step.id, layerId: step.id, itemVar: loop.for_each?.as || "item" };
}
function enclosingLoopVar(consumerId, varName, parentOf, stepById) {
...
function aggregateTo(stepId, targetSet, parentOf) {
  let cur = stepId;
  while (cur !== null) {
...
function projectWorkflow(ir3) {
  const tree = buildTreeProfile(ir3.steps);
  const shadowed = shadowedMap(tree);
  const stepById = /* @__PURE__ */ new Map();
  walkSteps(ir3.steps, (s) => stepById.set(s.id, s));
  ...
  const makeNode = (step, lane) => {
    ...
    const node = {
      ...
      lane,
      capture: typeof step.capture === "string" && step.capture ? step.capture : void 0,
  ...
  const projectLayer = (layerId, parentChain, containerKind, lanes, childrenOf, hostStep) => {
    const nodes = [];
    const edges = [];
    ...
    if (containerKind === "loop" && hostStep) {
      const entryId2 = SYNTH.entry(layerId);
      const scope = loopScopeOf(hostStep);
      nodes.unshift({
        id: entryId2,
        kind: "loop",
        category: "leaf",
        name: scope ? `\u5165\u53E3 \xB7 ${scope.itemVar}` : "\u5165\u53E3",
        lane: "main",
        synthetic: "entry",
      ...
      if (mainChildren.length > 0) {
        edges.push({
          id: `seq:${entryId2}->${mainChildren[0].id}`,
          source: entryId2,
          target: mainChildren[0].id,
          kind: "sequence"
        });
        const loopDef = hostStep.do.loop;
        edges.push({
    ...
    const dataEdgeKeys = /* @__PURE__ */ new Set();
    const pushDataEdge = (e) => {
      const key = `${e.source}\u2192${e.target}#${e.label ?? ""}#${e.dangling ? "d" : ""}`;
      if (dataEdgeKeys.has(key)) return;
      dataEdgeKeys.add(key);
      edges.push(e);
    };
    const externalAnchors = /* @__PURE__ */ new Map();
    const anchorFor = (varName, producerId) => {
      ...
      if (!anchor) {
        anchor = {
          ...
          synthetic: "external",
          externalVar: varName,
          externalProducerId: producerId ?? void 0
        };
      ...
      for (const cons of profile.consumes) {
        ...
        if (!producerId) {
          ...
          if (anchor.id !== consumerNode) {
            ...
            const cn2 = nodeById.get(consumerNode);
            if (cn2 && cn2.id === profile.stepId) {
              cn2.danglingVars = [...cn2.danglingVars ?? [], cons.varName];
            }
          }
          continue;
        }
    ...
    const outAnchors = /* @__PURE__ */ new Map();
    const outAnchorFor = (varName) => {
    ...
    if (layerId !== "root") {
      for (const profile of tree.order) {
        ...
        const hasOutsideConsumer = tree.order.some(
          (p2) => p2.consumes.some((c) => c.varName === profile.produces) && nearestProducer(tree, profile.produces, p2.stepId) === profile.stepId && !aggregateTo(p2.stepId, directIds, tree.parentOf)
        );
  ...
  projectLayer("root", [], "root", [{ id: "main", title: "\u4E3B\u6D41\u7A0B" }], () => ir3.steps, null);
  const index = {
...
// src/main-window/workflow-canvas/layout.ts
var SIZE = {
...
var LANE_GAP = 80;
function layerDir(layerId) {
  return layerId === "root" ? "LR" : "TB";
}
function layoutLane(nodes, edges, dir) {
...
function layoutLayer(layer, sidecarPos, dir = layerDir(layer.layerId)) {
...
// scripts/canvas-selftest.ts
var { readFileSync } = require("fs");
var { join } = require("path");
var WF_DIR = join(__dirname, "..", "..", "plugin", "workflows");
var failures = 0;
...
var ir2 = loadJson(join(WF_DIR, "nuphus-tour-v4", "workflow.json"));
var proj = projectWorkflow(ir2);
console.log("L0 Root \u5C42");
var root = proj.layers.get("root");
assert(!!root, "root \u5C42\u5B58\u5728");
var rootIds = root.nodes.map((n) => n.id);
assert(rootIds.includes("init") && rootIds.includes("tour"), "root \u5C42\u542B init/tour \u8282\u70B9");
var initNode = root.nodes.find((n) => n.id === "init");
assert(initNode.category === "container" && initNode.childCount === 2, "init \u662F seq \u5BB9\u5668 \xB7 2 \u6B65");
var rootPos = layoutLayer(root, void 0);
var initPos = rootPos.get("init");
var tourPos = rootPos.get("tour");
assert(
...
var tourNode = root.nodes.find((n) => n.id === "tour");
assert(
  tourNode.category === "container" && tourNode.childCount === 7 && (tourNode.containerSummary ?? "").includes("panels") && (tourNode.containerSummary ?? "").includes("p"),
  `tour \u662F loop \u5BB9\u5668 \xB7 7 \u6B65 \xB7 for_each panels as p\uFF08\u6458\u8981: ${tourNode.containerSummary}\uFF09`
);
assert(
  root.edges.some((e) => e.kind === "sequence" && e.source === "init" && e.target === "tour"),
  "\u987A\u5E8F\u8FB9 init \u2192 tour"
);
var crossEdge = root.edges.find(
...
var initLayer = proj.layers.get("init");
...
var legacy = loadJson(join(WF_DIR, "nuphus-tour-v4", "nuphus-tour-v4.json"));
var legacyProj = projectWorkflow(legacy);
assert(legacyProj.index.hasCustomNodes, "\u65E7\u683C\u5F0F\u88AB\u8BC6\u522B\u4E3A Custom\uFF08\u6574\u6811\u53EA\u8BFB + \u8FC1\u79FB\u6A2A\u5E45\uFF09");
var legacyRoot = legacyProj.layers.get("root");
assert(
  legacyRoot.nodes.every((n) => n.category === "unknown"),
  "\u65E7\u683C\u5F0F\u8282\u70B9\u5168\u90E8\u6E32\u67D3\u4E3A unknown\uFF08\u4E0D\u517C\u5BB9\uFF09\u7C7B\u522B"
);
console.log("chatagent-opencode-verify");
var chat = loadJson(join(WF_DIR, "chatagent-opencode-verify", "workflow.json"));
var chatProj = projectWorkflow(chat);
