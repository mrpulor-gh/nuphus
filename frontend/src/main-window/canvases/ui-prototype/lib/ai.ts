...
const STORE_KEY = 'nuphus.ui_proto.ai'
...
const isLocal = (u: string) =>
  /^https?:\/\/(localhost|127\.0\.0\.1|\[::1\])(:|\/|$)/i.test(u.trim())
...
const itemsOnFrame = (doc: Doc, frame: Frame, widths: Record<string, number>): Item[] =>
  doc.groups
    .filter((g: Group) => frameOfGroup(g, doc.frames, widths)?.id === frame.id)
    .flatMap(g => g.items)
...
const SYSTEM =
  'You help an app designer finish a sketch. Answer with a single JSON object and nothing else: no prose, no markdown fence.'
...
export async function proposeBehavior(
  s: AiSettings,
  doc: Doc,
...
export async function proposeDescription(
  ...
  const user = [
    ...
    hasText(frame.note)
      ? `The author's current description is ${JSON.stringify(frame.note)}: keep its intent and facts, and improve it.`
      : '',
    `Write in ${LANG_NAME[lang]}.`,
    '',
...
export function popHistory<V extends string, H extends string>(
  current: string | undefined,
  history: string[] | undefined,
  valueKey: V,
  historyKey: H,
): Record<V, string> & Record<H, string[] | undefined> {
  const [prev] = history ?? []
  const cur = (current ?? '').trim()
  return { [valueKey]: prev ?? '', [historyKey]: cur ? [cur] : undefined } as Record<V, string> &
    Record<H, string[] | undefined>
}
...
export async function draftDesign(
  ...
  const system = [
    'You draft UI Prototype canvas designs. Follow the guide below exactly.',
