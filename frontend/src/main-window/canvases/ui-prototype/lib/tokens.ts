...
export const FONTS: {
  ...
  label: string
  family: string
  /** Google Fonts family to fetch, if any */ google?: string
}[] = [
  { key: 'roboto', label: 'Roboto', family: 'Roboto, system-ui, sans-serif' },
  {
    key: 'robotoFlex',
    label: 'Roboto Flex',
    family: "'Roboto Flex', Roboto, system-ui, sans-serif",
    google: 'Roboto+Flex:wght@400;500;600;700',
  },
  {
    key: 'robotoSerif',
    label: 'Roboto Serif',
    family: "'Roboto Serif', Georgia, serif",
    google: 'Roboto+Serif:wght@400;500;600;700',
  },
...
export function paletteOf(key: string, custom?: Palette | null, theme?: Theme): Palette {
  ...
  return {
    ...schemeFromSeed(seed, base.label, {
      dark: theme.dark,
      contrast: theme.contrast,
      keepChroma: base.key !== 'custom',
    }),
    key: base.key,
  }
}
...
export const KIND_SPEC: Record<Kind, KindSpec> = {
  ...
  image: {
    ...
    size: {
      ...
      step: 4,
      icon: 'open_in_full',
      presets: [96, HALF_W, CONTENT_W, PHONE_W],
    },
    defLabel: '',
    defIcon: 'image',
    defSize: 200,
...
export const SLIDE_SPEC: Partial<
  Record<Transition, { axis: 'x' | 'y'; enter: number; exit: number }>
> = {
...
export type Transition =
  'slide' | 'slideLeft' | 'slideUp' | 'slideDown' | 'fade' | 'expand' | 'none'
...
export function actionSlotsOf(it: Item): IconSlot[] {
  if (
    ...
    return iconSlotsOf(it).filter(s => !!s.value)
  if (it.kind === 'fabMenu')
...
export const toggleIcon = (it: Item): string | null =>
  it.toggle && it.toggle.icon !== undefined ? it.toggle.icon : it.icon
...
export function collapseFree(g: Group, widths: Record<string, number>): Group {
  ...
  return {
    ...
    x: pl.x,
    y: pl.y,
    axis: connectSpecOf(g.items[0])?.axis ?? 'x',
    items: g.items,
  }
}
...
export const cardFillOf = (it: Item): ColorToken =>
  it.fill ??
  (it.variant === 'outlined'
    ? 'surface'
    : it.variant === 'elevated'
...
export function carryItemSize(
  ...
  const keepsShape =
    it.kind === 'card' || it.kind === 'image' || it.kind === 'camera' || it.kind === 'map'
...
export function explodeGroup(g: Group, widths: Record<string, number>): Group[] {
  ...
  for (const start of placed) {
    ...
    for (;;) {
      const next = placed.find(
        q =>
          ...
          (axis === 'x'
            ? near(q.y, last.y) && near(q.x, last.x + last.w + GAP)
            : near(q.x, last.x) && near(q.y, last.y + last.h + GAP)),
      )
      if (!next) break
    ...
    const anchor = run.reduce((a, b) =>
      (rank.get(a.item.id) ?? 0) <= (rank.get(b.item.id) ?? 0) ? a : b,
    )
    out.push({
      id: `${g.id}:${anchor.item.id}`,
      x: start.x,
      y: start.y,
      axis,
...
export function freeRadii(g: Group, widths: Record<string, number>): Map<string, Radii> {
  ...
  for (const run of explodeGroup(g, widths)) {
    ...
    run.items.forEach((it, i) => {
      ...
      out.set(
        it.id,
        c ? runCorners(run.axis, i === 0, i === n - 1, c.outer, c.inner) : baseRadii(it),
      )
    })
  }
  return out
...
export const defaultPlatformOf = (frames: Frame[], mode: FrameMode): Platform =>
  mode === 'phone' && frames.some(f => !isPhoneFrame(f)) ? 'web' : DEFAULT_PLATFORM
...
export function baseRadii(it: Item): Radii {
  ...
  switch (it.kind) {
    ...
    case 'topAppBar':
    case 'tabs': {
      /* box without typed corners shares the top/bottom radius behaviour of the bar parts */
      if (it.kind === 'box' && it.corners) return { ...it.corners }
