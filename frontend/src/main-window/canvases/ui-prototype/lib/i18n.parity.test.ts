...
function leafPaths(value: unknown, prefix = ''): string[] {
  if (value !== null && typeof value === 'object') {
    return Object.entries(value)
      .flatMap(([key, child]) => leafPaths(child, `${prefix}.${key}`))
      .sort()
  }
  return [prefix]
}
  ...
  it('keeps template placeholders consistent across languages', () => {
    ...
    for (const key of keys) {
      for (const lang of languages)
        expect(placeholders(t(key, lang)), `${key}.${lang}`).toEqual(placeholders(t(key, 'en')))
    }
  })
})
...
describe('dictionary coverage of editor tokens', () => {
  ...
  it('keeps navigation and FAB icons aligned by translation index', () => {
    for (const table of [NAV_TABS, FAB_MENU_TABS]) {
      for (const lang of languages)
        expect(
          table[lang].map(({ icon }) => icon),
          lang,
        ).toEqual(table.en.map(({ icon }) => icon))
    }
  })
})
...
describe('LANG_FONT coverage', () => {
  ...
  it.each([
    ...
    'supplies the matching Noto font family and download query for %s',
    (lang, script) => {
