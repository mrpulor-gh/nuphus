...
import { Doc, Frame, Group, Item, Palette, PHONE_W, PHONE_H, makeItem } from './tokens'
...
describe('buildPrompt — behavior / actions', () => {
  it("an item with action.to='back' generates a 'go back' note", () => {
    const item: Item = {
      ...
      icon: null,
      variant: 'filled',
      action: { to: 'back', transition: 'none' },
    }
    const doc = baseDoc({ groups: [{ id: 'g', x: 100, y: 100, axis: 'x', items: [item] }] })
    const out = buildPrompt(doc, widths, undefined, 'en')
      ...
      groups: [
        {
          ...
          y: 100,
          axis: 'x',
          items: [{ id: 'a', kind: 'button', label: 'B1', icon: null, variant: 'filled' }],
        },
        {
          id: 'g2',
          x: 700,
          y: 100,
          axis: 'x',
          items: [{ id: 'b', kind: 'button', label: 'B2', icon: null, variant: 'filled' }],
        },
      ],
    })
    const out = buildPrompt(doc, widths, undefined, 'en')
