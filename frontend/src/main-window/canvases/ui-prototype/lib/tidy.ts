...
type Unit = {
  ...
  kind: string
  checked?: boolean
  /** the first part, for family checks */ probe: Item
}
...
const share = (a0: number, a1: number, b0: number, b1: number) =>
  ...
  for (;;) {
    ...
    outer: for (let i = 0; i < out.length; i++) {
      ...
      for (let j = 0; j < out.length; j++) {
        ...
        const ahead =
          fa.axis === 'x'
            ? ra.l < rb.l || (ra.l === rb.l && i < j)
            : ra.t < rb.t || (ra.t === rb.t && i < j)
        if (gap < -2 && !ahead) continue
        const lined =
          fa.axis === 'x' ? share(ra.t, ra.b, rb.t, rb.b) : share(ra.l, ra.r, rb.l, rb.r)
        if (lined < 0.5) continue
        /* nothing else may sit between them */
        const between: Rect =
          fa.axis === 'x'
            ? { l: ra.r, t: Math.min(ra.t, rb.t), r: rb.l, b: Math.max(ra.b, rb.b) }
            : { l: Math.min(ra.l, rb.l), t: ra.b, r: Math.max(ra.r, rb.r), b: rb.t }
  ...
  for (;;) {
    ...
    outer: for (let i = 0; i < units.length; i++) {
      ...
      for (let j = i + 1; j < units.length; j++) {
        ...
        units[i] = {
          ...big,
          ids: [...units[i].ids, ...units[j].ids],
          bb: union(units[i].bb, units[j].bb),
        }
        units.splice(j, 1)
        merged = true
        break outer
...
const isControl = (u: Unit) =>
  u.kind === 'switch' || u.kind === 'checkbox' || u.kind === 'radio' || u.kind === 'iconButton'
const isAnchored = (u: Unit) =>
  isRail(u) || isTop(u) || isBottomBar(u) || isFloatingBottom(u) || isFab(u) || isOverlay(u)
...
function gapBefore(prev: Unit[] | null, row: Unit[]): number {
  ...
  if (
    prev.length === 1 &&
    row.length === 1 &&
    canJoin(prev[0].probe, row[0].probe) &&
    connectSpecOf(prev[0].probe)?.axis === 'y'
  )
    return APART_GAP_Y
...
export function pullInto(g: Group, frame: Frame, widths: Record<string, number>): Group {
  ...
  if (g.items.length === 1 && FULL_WIDTH.includes(g.items[0].kind) && bb.r - bb.l >= fr.r - fr.l)
    dx = fr.l - bb.l
...
export function carryFrame(
  ...
  const moved = new Map(
    frames
      .filter(f => f.id !== frame.id && f.x >= frame.x + from.w)
      .map(f => [f.id, shift] as const),
  )
  const nextFrames = frames.map(f =>
    f.id === to.id ? to : moved.has(f.id) ? { ...f, x: f.x + shift } : f,
  )
  ...
  const swapNav = (g: Group, it: Item): Item => {
    ...
    if (expanded && it.kind === 'bottomNav')
      return {
        ...it,
        kind: 'navRail',
        size: undefined,
        size2: after.h,
        radiusTop: it.radiusBottom,
        radiusBottom: it.radiusTop,
      }
  ...
  const side =
    navGroup && standsAlone(navGroup, 'navRail') ? railSide(navGroup, frame, widths) : 'left'
  ...
  const resized = groups.map(g => {
    ...
    if (o === frame.id) {
      ...
      const items = g.items.map(it =>
        swapNav(g, carryItemSize(it, isRail ? from : fromBody, isRail ? after : toBody)),
      )
      const x = isRail
        ? side === 'right'
          ? to.x + after.w - RAIL_W
          : to.x
        : g.x + leftAfter - leftBefore
...
export function barSlotOf(
  ...
  const rails = groups.filter(
    g =>
      frameOfGroup(g, frames, widths)?.id === frame.id &&
      g.items.length === 1 &&
      g.items[0].kind === 'navRail',
  )
...
function snapEdges(
  ...
  const container = runs.reduce((a, b) =>
    area(rects.get(a.id)!) >= area(rects.get(b.id)!) ? a : b,
  )
  ...
  const nudge = (g: Group, dx: number, dy: number) => {
    ...
    groups.set(g.id, {
      ...groups.get(g.id)!,
      x: groups.get(g.id)!.x + dx,
      y: groups.get(g.id)!.y + dy,
    })
    moved = true
  }
  const lefts = commonValue(
    others.map(g => rects.get(g.id)!.l),
    SNAP,
  )
  const rights = commonValue(
    others.map(g => rects.get(g.id)!.r),
    SNAP,
  )
  ...
  for (const g of others) {
    ...
    const beside = others
      .filter(o => o !== g && share(r.t, r.b, rects.get(o.id)!.t, rects.get(o.id)!.b) > 0.5)
      .map(o => groupBounds(groups.get(o.id)!, widths).t)
    const tops = beside.length === 1 ? beside[0] : commonValue(beside, SNAP)
    if (tops !== null && r.t !== tops && Math.abs(r.t - tops) <= SNAP)
      nudge(g, 0, Math.round(tops - r.t))
  }
  return moved
}
...
function evenCorners(groups: Map<string, Group>): boolean {
  ...
  for (const g of groups.values())
    for (const it of g.items) {
      ...
      else if (it.kind === 'box' && !it.corners && (it.radiusTop ?? 0) === (it.radiusBottom ?? 0))
        boxes.push({ g, it, r: it.radiusTop ?? 0 })
    }
  let changed = false
      ...
      groups.set(g.id, {
        ...
        items: cur.items.map(x =>
          x.id === it.id
            ? x.kind === 'card'
              ? { ...x, radiusTop: common }
              : { ...x, radiusTop: common, radiusBottom: common }
            : x,
        ),
      })
      changed = true
...
export function bodyRect(
  ...
  const mine = groups.filter(
    g => !except.has(g.id) && frameOfGroup(g, frames, widths)?.id === frame.id,
  )
  ...
  return {
    l,
    t,
    r: Math.max(l, screen.r - rightRails * RAIL_W - PHONE_MARGIN),
    b: Math.max(t, bottom - PHONE_MARGIN),
  }
}

export function tidyFrame(
  groups: Group[],
  frame: Frame,
  frames: Frame[],
  widths: Record<string, number>,
  ...
  const mineIds = new Set(
    groups.filter(g => frameOfGroup(g, frames, widths)?.id === frame.id).map(g => g.id),
  )
  ...
  const fr: Rect = {
    ...screen,
    l: screen.l + leftRails.length * RAIL_W,
    r: screen.r - rightRails.length * RAIL_W,
  }
  ...
  for (const u of units.filter(isBottomBar).sort((a, b) => b.bb.t - a.bb.t)) {
    ...
    target.set(u, {
      l: fr.l + Math.max(0, Math.round((frameW - (u.bb.r - u.bb.l)) / 2)),
      t: bottom,
    })
  }
  ...
  for (const u of units.filter(isOverlay)) {
    target.set(u, {
      l: fr.l + Math.round((frameW - (u.bb.r - u.bb.l)) / 2),
      t: fr.t + Math.round((frameH - (u.bb.b - u.bb.t)) / 2),
    })
  }
  ...
  const even = spreading
    ? (spare + used - heights.reduce((a, h) => a + h, 0)) / (laid.length + 1)
    : 0
  const offset = !fits
    ? 0
    : place === 'center' || (place === 'spread' && laid.length <= 1)
      ? Math.round(spare / 2)
      : place === 'bottom'
        ? spare
        : 0
  ...
  laid.forEach(({ row, y: rowY, rowH }, index) => {
    ...
    if (row.length === 1) {
      ...
      target.set(label, {
        l: fr.l + PHONE_MARGIN,
        t: yy + Math.round((rowH - (label.bb.b - label.bb.t)) / 2),
      })
      target.set(control, {
        l: fr.r - PHONE_MARGIN - cw,
        t: yy + Math.round((rowH - (control.bb.b - control.bb.t)) / 2),
      })
    } else {
      ...
      const gaps = row
        .slice(1)
        .map((u, i) => (canJoin(row[i].probe, u.probe) ? APART_GAP_X : ROW_ITEM_GAP))
      ...
      const a = spread
        ? 'left'
        : align({ l: row[0].bb.l, t: 0, r: row[row.length - 1].bb.r, b: 0 }, fr)
      let x =
        a === 'right'
          ? fr.r - PHONE_MARGIN - packed
          : a === 'center'
            ? fr.l + (frameW - packed) / 2
            : fr.l + PHONE_MARGIN
      row.forEach((u, i) => {
        const h = u.bb.b - u.bb.t
