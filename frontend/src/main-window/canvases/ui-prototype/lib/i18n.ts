...
export function translateDefaultText(
  value: string,
  kind: string,
  field: 'label' | 'supporting' | 'tab',
  lang: Lang,
): string {
    ...
    if (field === 'tab') {
      const labels = (l: Lang) =>
        kind === 'tabs'
          ...
          : kind === 'select'
            ? SELECT_OPTIONS[l]
            : (kind === 'fabMenu' ? FAB_MENU_TABS[l] : NAV_TABS[l]).map(tab => tab.label)
      const index = labels(from).indexOf(value)
      if (index >= 0) return labels(lang)[index] ?? value
    } else {
      if (value && value === KIND_TEXT[from][kind]?.[field])
        return KIND_TEXT[lang][kind]?.[field] ?? value
      const keys: (keyof typeof SEED_TEXT.en)[] =
        field === 'supporting'
          ? ['supporting']
          : kind === 'button'
            ? ['favorite', 'share', 'start']
            : kind === 'listItem'
...
export const COLOR_TOKEN_TEXT = {
  ja: {
    surface: 'サーフェス',
    surfaceContainerLow: 'コンテナ（低）',
    surfaceContainer: 'コンテナ',
    surfaceContainerHigh: 'コンテナ（高）',
    surfaceContainerHighest: 'コンテナ（最高）',
    primaryContainer: 'プライマリコンテナ',
    secondaryContainer: 'セカンダリコンテナ',
    tertiaryContainer: 'ターシャリコンテナ',
    primary: 'プライマリ',
    inverseSurface: '反転サーフェス',
  },
  zh: {
    surface: '表面',
  ...
  ko: {
    surface: '표면',
    surfaceContainerLow: '낮은 컨테이너',
    surfaceContainer: '컨테이너',
    surfaceContainerHigh: '높은 컨테이너',
    surfaceContainerHighest: '가장 높은 컨테이너',
    primaryContainer: '주 색상 컨테이너',
    secondaryContainer: '보조 색상 컨테이너',
    tertiaryContainer: '세 번째 색상 컨테이너',
    primary: '주 색상',
    inverseSurface: '반전 표면',
  },
}
...
export const UI = {
  ...
  askAiIdea: {
    ja: '作りたいもの（例: レシピを保存して検索できるアプリ）',
    en: 'What to build (e.g. an app to save and search recipes)',
    zh: '想做的东西（例如：能保存和搜索菜谱的应用）',
  },
  askAiIdeaFallback: {
    ja: '（ここに作りたいものを書く）',
    en: '(describe what to build here)',
    zh: '（在这里写下想做的东西）',
  },
  askAiCopy: { ja: '指示をコピー', en: 'Copy the instruction', zh: '复制指令' },
  askAiText: {
    ja: 'UI Prototype canvas のスケッチを作ってください。まず {url} を読み、その手順どおりに設計を JSON で組み立てて、共有リンクの形で返してください。コードを実行できない場合は、設計の JSON をコードブロックで返してください（ファイルに保存して読み込みます）。検証は不要です。\n\n作りたいもの: {idea}',
    en: 'Make an UI Prototype canvas sketch. First read {url} and follow it: build the design as JSON and reply with a share link. If you cannot run code, reply with the JSON in a code block (it will be saved to a file and opened). No verification is needed.\n\nWhat to build: {idea}',
    zh: '请制作一个 UI Prototype canvas 草图。先阅读 {url}，按其中的步骤用 JSON 组织设计，并以分享链接的形式回复。如果无法运行代码，就在代码块中回复设计的 JSON（会保存为文件后打开）。不需要验证。\n\n想做的东西：{idea}',
  },
  askAiGenerate: { ja: 'AI で作る', en: 'Draft with AI', zh: '让 AI 生成' },
  askAiPasted: {
    ja: 'AI エージェントに貼り付けてください',
    en: 'Paste it into your AI agent',
    zh: '请粘贴到 AI 代理中',
  },
  askAiTitle: { ja: 'AI に設計を頼む', en: 'Ask an AI for a design', zh: '让 AI 设计' },
  askAiCopyTitle: {
    ja: 'AI エージェント用の指示',
    en: 'Instruction for an AI agent',
    zh: 'AI 代理用指令',
  },
  askAiGenerateTitle: {
    ja: 'AI タブのキーで作る',
    en: 'Uses the key in the AI tab',
    zh: '使用 AI 标签页的密钥',
  },
  aiSetup: { ja: 'AI の設定', en: 'AI settings', zh: 'AI 设置' },
  aiSetupHint: {
    ja: 'キーを入れると、ここで直接作れます',
    en: 'Add a key to draft it right here',
    zh: '填写密钥后可在此直接生成',
  },
  ...
  shareLinkHint: {
    ja: 'この設計を開くリンク（画像は含まない）',
    en: 'A link that opens this design (no images)',
    zh: '打开此设计的链接（不含图片）',
  },
  replaceProject: {
  ...
  selectedOption: {
    ja: '初期値にする（もう一度押すと未選択）',
    en: 'Make this the initial value (press again for none)',
    zh: '设为初始值（再按一次取消）',
  },
  ...
  onStateHint: {
    ja: 'オンのときの文字・アイコン・スタイル',
    en: 'Text, icon and style when on',
    zh: '开启时的文字、图标和样式',
  },
  groupEditNote: {
    ja: '中の部品を編集するにはグループを解除してください',
    en: 'Ungroup to edit the parts inside',
    zh: '要编辑其中的组件，请先取消编组',
  },
  ...
  targetAndroid: {
    ja: 'Android のネイティブアプリとして作る',
    en: 'Build as a native Android app',
    zh: '作为 Android 原生应用构建',
  },
  targetWeb: {
    ja: 'ブラウザで動く Web アプリとして作る',
    en: 'Build as a web app that runs in the browser',
    zh: '作为在浏览器中运行的 Web 应用构建',
  },
  copyPrompt: { ja: 'プロンプトをコピー', en: 'Copy prompt', zh: '复制提示词' },
  downloadPromptPng: { ja: 'PNG で書き出す', en: 'Download PNG', zh: '下载原型图' },
  sendToLeader: { ja: 'Leader に送る', en: 'Send to Leader', zh: '发送给 Leader' },
  sentToLeader: {
    ja: 'Leader に送信しました',
    en: 'Sent to Leader',
    zh: '已发送给 Leader',
  ...
  imageSavedToBrowser: {
    ja: 'ブラウザの既定のダウンロード先に保存しました',
    en: 'Saved to your browser download folder',
    zh: '已下载到浏览器默认下载目录',
  },
  capturingImage: {
    ja: '画像を生成しています…',
  ...
  sendingToLeader: {
    ja: 'プロトタイプを生成して送信しています…',
    en: 'Capturing the prototype and sending…',
    zh: '正在生成原型图并发送…',
  },
  imageTooLargeTextOnly: {
    ja: '画像が大きすぎるため、テキストのみ送信します',
    en: 'Image too large - sending the text only',
    zh: '图片过大，已仅发送文本',
  },
  sendFailedToLeader: {
    ja: 'Leader への送信に失敗しました',
    en: 'Failed to send to Leader',
    zh: '发送给 Leader 失败',
  },
  sendStatusUnknown: {
    ja: '送信状態が不明です。チャットを確認してください',
    en: 'Send status unknown - check the chat',
    zh: '发送状态未知，请查看对话',
  },
  sendEmptyPrompt: {
    ja: '送信するプロンプトが空です',
  ...
  // mobile
  mobileNote: {
    ja: 'フル機能は PC のブラウザで使えます',
    en: 'Full features on a desktop browser',
    zh: '完整功能请在电脑浏览器中使用',
  },
  ...
  alignHintOne: {
    ja: '画面の本文領域（バーとレールを除いた余白の内側）に揃えます。',
    en: "Lines the part up with the screen's body, inside the margins and clear of the bars.",
    zh: '与屏幕正文区域（栏和导轨以外、边距以内）对齐。',
  },
  alignHintMany: {
  ...
  promptReset: {
    ja: '生成されたプロンプトに戻す',
    en: 'Back to the generated prompt',
    zh: '恢复为生成的提示词',
  },
  ...
  aiRestore: {
    ja: 'AI の前と切り替える',
    en: 'Switch between the AI rewrite and the original',
    zh: '在 AI 改写与原文之间切换',
  },
  aiApplied: { ja: '適用しました', en: 'Applied', zh: '已应用' },
  aiSelectScreen: {
    ja: '先に画面を選んでください',
    en: 'Select a screen first',
    zh: '请先选择一个屏幕',
  },
  aiNoKey: {
    ja: 'AI タブでキーを入れると使えます',
    en: 'Add a key in the AI tab to use this',
    zh: '在 AI 标签页中填写密钥后即可使用',
  },
  ...
  aiErrorJson: {
    ja: 'モデルの返答を読み取れませんでした',
    en: "The model's reply could not be read",
    zh: '无法解析模型的回复',
  },
  aiErrorLong: {
    ja: '返答が長すぎて途中で切れました。画面を減らして試してください',
    en: 'The reply was cut short. Try fewer screens',
    zh: '回复过长被截断了。请减少屏幕数量再试',
  },
  aiErrorModel: { ja: 'モデル ID を入力してください', en: 'Enter a model ID', zh: '请输入模型 ID' },
  aiErrorInsecure: {
    ja: 'ベース URL は https か localhost にしてください',
    en: 'The base URL must use https or point at localhost',
    zh: '基础 URL 必须使用 https 或指向 localhost',
  },
  aiErrorNetwork: {
...
export const KO: Record<UIKey, string> = {
  ...
  phoneFrame: '휴대전화',
  desktopFrame: '데스크톱',
  columnWidth: '휴대전화 한 화면 너비',
  cornerLeft: '왼쪽 모서리',
  cornerRight: '오른쪽 모서리',
  cornersEach: '모서리별로 지정',
  cornerTl: '왼쪽 위',
  cornerTr: '오른쪽 위',
  cornerBl: '왼쪽 아래',
  cornerBr: '오른쪽 아래',
  ...
  clearAll: '모두 지우기',
  clearAllTitle: '캔버스를 비울까요?',
  clearAllBody: '모든 화면과 부품을 삭제합니다. 실행 취소(Ctrl+Z)로 복원할 수 있습니다.',
  screen: '화면',
  screenName: '화면 이름',
  name: '이름',
  background: '배경',
  ...
  saveProject: '프로젝트 저장',
  openProject: '프로젝트 열기',
  replaceProjectTitle: '이 프로젝트를 열까요?',
  replaceProject: '현재 캔버스가 교체됩니다. 실행 취소(Ctrl+Z)로 되돌릴 수 있습니다.',
  askAi: 'AI에게 맡기기',
  askAiHint:
    "만들고 싶은 것을 적고 'AI로 만들기'를 누르면 AI 탭의 키로 설계가 캔버스에 그려집니다. 대신 지시를 복사해 Claude Code 같은 AI 에이전트에 넘길 수도 있습니다. 답한 링크를 열거나 JSON을 파일로 저장해 '프로젝트 열기'로 불러오세요.",
  askAiGenerate: 'AI로 만들기',
  askAiGenerating: '만드는 중…',
  askAiPasted: 'AI 에이전트에 붙여 넣으세요',
  askAiTitle: 'AI에게 설계 맡기기',
  askAiCopyTitle: 'AI 에이전트용 지시',
  askAiGenerateTitle: 'AI 탭의 키로 만들기',
  aiSetup: 'AI 설정',
  aiSetupHint: '키를 입력하면 여기서 바로 만들 수 있습니다',
  aiSetupTitle: 'AI 탭 열기',
  draftKeep: '이 설계 채택',
  draftUndo: '이전 설계로 되돌리기',
  askAiIdea: '만들고 싶은 것(예: 레시피를 저장하고 검색하는 앱)',
  askAiIdeaFallback: '(여기에 만들고 싶은 것을 적으세요)',
  askAiCopy: '지시 복사',
  askAiText:
    'UI Prototype canvas 스케치를 만들어 주세요. 먼저 {url}을 읽고 그 절차대로 설계를 JSON으로 구성한 뒤, 공유 링크 형태로 답해 주세요. 코드를 실행할 수 없다면 설계 JSON을 코드 블록으로 답해 주세요(파일로 저장해 불러옵니다). 검증은 필요 없습니다.\n\n만들고 싶은 것: {idea}',
  selectedTab: '선택 항목으로 지정',
  shareLinkCopy: '링크 복사',
  shareLinkHint: '이 설계를 여는 링크(이미지 제외)',
  invalidProject: '프로젝트 파일을 열 수 없습니다.',
  readOnlyTitle: '다른 탭에서 편집 중입니다',
  readOnlyBody:
    '이 캔버스는 다른 탭에서 편집 중입니다. 해당 탭을 닫은 다음 이 페이지를 새로고침하세요.',
  reload: '새로고침',
  copied: '복사됨',
  saveImage: '이미지로 저장',
  saving: '저장 중…',
  previewFrom: '이 화면부터 미리보기',
  duplicate: '복제',
  duplicateKey: '복제 (Ctrl+D)',
  delete: '삭제 (Delete)',
  ...
  options: '옵션',
  addOption: '옵션 추가',
  removeOption: '이 옵션 삭제',
  selectedOption: '초깃값으로 설정(다시 누르면 선택 해제)',
  image: '이미지',
  pickImage: '이미지 선택',
  removeImage: '이미지 제거',
  imageUrl: '이미지 URL',
  ...
  cornerTop: '위쪽 모서리',
  cornerBottom: '아래쪽 모서리',
  screenWidth: '화면 너비',
  contentWidth: '좌우 16dp 여백',
  halfWidth: '한 행의 절반(2열)',
  screenHeight: '화면 높이',
  halfHeight: '화면의 절반',
  tapTo: '탭하여 이동',
  none: '없음',
  ...
  swipeTo: '스와이프하여 이동',
  toggle: '토글 버튼',
  toggleHint: '탭할 때 켜짐/꺼짐 전환',
  thumbCheck: '켜졌을 때 체크 아이콘 표시',
  behavior: '동작',
  whenPressed: '눌렀을 때…',
  whatItDoes: '이 부품의 동작…',
  removeLink: '링크 제거',
  ...
  ungroup: '그룹 해제',
  selectedParts: '개 선택됨',
  groupHint: '겹침을 유지한 채 하나의 레이어처럼 함께 이동합니다',
  iconBackground: '아이콘 배경',
  noBackground: '배경 없음',
  normalState: '기본',
  onState: '켜짐',
  onStateHint: '켜졌을 때의 텍스트, 아이콘, 스타일',
  groupEditNote: '안쪽 부품을 편집하려면 그룹을 해제하세요',
  openPanel: '패널 열기',
  colors: '색상',
  templates: '팔레트',
  customColor: '사용자 지정',
  seedColor: '기준 색상',
  seedHint:
    '색상 하나로 전체 Material 3 색상 구성을 만듭니다. 세부 조정에서 개별 색상도 바꿀 수 있습니다.',
  useThis: '이 색상 사용',
  fineTune: '세부 조정',
  dynamicColor: '동적 색상',
  dynamicOnHint:
    '여기 표시된 색상은 편집기 전용입니다. 실제 기기에서는 배경화면 색상을 사용합니다.',
  dynamicOffHint: '켜면 실제 기기는 배경화면 색상을 사용하고 여기의 색상은 대체 색상이 됩니다.',
  closeBtn: '닫기',
  screens: '화면 선택',
  noLayers: '이 화면에는 아직 부품이 없습니다',
  showParts: '안의 부품 표시',
  hideParts: '안의 부품 숨기기',
  brief: '이 앱에 대한 설명…',
  appName: '앱 이름',
  targetPlatform: '구현 대상',
  targetAndroid: 'Android 네이티브 앱으로 만들기',
  targetWeb: '브라우저에서 실행되는 웹 앱으로 만들기',
  copyPrompt: '프롬프트 복사',
  ...
  imageSavedTo: '이미지 저장 위치',
  imageSavedToBrowser: '브라우저 기본 다운로드 폴더에 저장했습니다',
  capturingImage: '이미지를 생성하는 중…',
  sendingToLeader: '프로토타입을 생성해 전송하는 중…',
  imageTooLargeTextOnly: '이미지가 너무 커서 텍스트만 전송합니다',
  ...
  screenN: '화면',
  copySuffix: ' 복사본',
  mobileNote: '전체 기능은 데스크톱 브라우저에서 사용할 수 있습니다',
  addButton: '버튼 추가',
  done: '완료',
  theme: '테마',
  settings: '테마 및 설정',
  ...
  shapeRounded: '둥근형',
  shapeFull: '완전 둥근형',
  shapeHint: '모든 부품의 기본 모서리를 한 번에 바꿉니다. 부품에 직접 입력한 반경은 유지됩니다.',
  fontFamily: '글꼴',
  emphasized: '강조 스타일',
  emphasizedHint: '제목과 레이블에 더 굵은 M3 Expressive 스타일을 사용합니다.',
  motionScheme: '모션 방식',
  motionStandard: '표준',
  motionExpressive: '익스프레시브',
  motionHint:
    '익스프레시브는 통통 튀는 스프링 효과입니다. 미리보기 화면 전환과 프롬프트에 반영됩니다.',
  tryIt: '탭하여 확인',
  tidy: '정리',
  tidyUndo: '정리 실행 취소',
  tidyDone: '이미 정돈되어 있습니다',
  placement: '본문의 세로 배치',
  placeTop: '위에서부터',
  placeCenter: '가운데',
  placeBottom: '아래쪽',
  placeSpread: '균등',
  align: '정렬',
  alignHintOne: '화면 본문 영역(바와 레일을 제외한 여백 안쪽)에 맞춰 정렬합니다.',
  alignHintMany: '선택한 부품끼리 정렬합니다. 균등 배치는 양끝 부품을 고정합니다.',
  alignLeft: '왼쪽 정렬',
  alignCenterH: '가로 가운데 정렬',
  alignRight: '오른쪽 정렬',
  distributeH: '가로 균등 배치',
  alignTop: '위쪽 정렬',
  alignCenterV: '세로 가운데 정렬',
  alignBottom: '아래쪽 정렬',
  distributeV: '세로 균등 배치',
  description: '설명',
  screenDescription: '이 화면의 용도',
  ai: 'AI',
  promptReset: '생성된 프롬프트로 되돌리기',
  aiWriteShort: 'AI로 작성',
  aiWrite: 'AI에게 작성 맡기기',
  aiSettings: 'AI 설정',
  aiProvider: '제공업체',
  ...
  aiKey: 'API 키',
  aiGetKey: '키 받기',
  aiKeyHint: '키는 이 브라우저에만 저장되며 제공업체로 직접 전송됩니다.',
  aiRestore: 'AI 수정본과 원본 전환',
  aiApplied: '적용됨',
  aiSelectScreen: '먼저 화면을 선택하세요',
  aiNoKey: 'AI 탭에 키를 입력하면 사용할 수 있습니다',
  aiError: 'AI 요청에 실패했습니다',
  aiErrorRefusal: '모델이 답변을 거부했습니다',
  aiErrorJson: '모델의 응답을 읽을 수 없습니다',
  aiErrorLong: '답변이 너무 길어 중간에 잘렸습니다. 화면 수를 줄여 다시 시도하세요',
  aiErrorModel: '모델 ID를 입력하세요',
  aiErrorInsecure: '기본 URL은 https를 사용하거나 localhost를 가리켜야 합니다',
  aiErrorNetwork: '연결할 수 없습니다. URL, 네트워크 및 서버의 CORS 설정을 확인하세요',
}

export const t = (key: UIKey, lang: Lang = current): string =>
  lang === 'ko' ? KO[key] : UI[key][lang]
