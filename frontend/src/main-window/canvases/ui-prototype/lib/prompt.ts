...
function cardImage(it: Item, lang: Lang): string {
  ...
  if (lang === 'ja')
    return url
      ...
      : it.src
        ? '上部に指定の画像、'
        : `上部に${it.icon ? `${it.icon} アイコンの` : ''}プレースホルダー画像、`
  if (lang === 'zh')
    return url
      ? `顶部是 ${url} 的图片，`
      : it.src
        ? '顶部是指定的图片，'
        : `顶部是${it.icon ? `${it.icon} 图标的` : ''}占位图片，`
  if (lang === 'ko')
    return url
      ? `위쪽에 ${url}의 이미지, `
      : it.src
        ? '위쪽에 지정한 이미지, '
        : `위쪽에 ${it.icon ? `${it.icon} 아이콘의 ` : ''}자리표시자 이미지, `
  return url
    ? `with an image from ${url} on top, `
    : it.src
      ? 'with the provided image on top, '
      : `with a placeholder image${it.icon ? ` (${it.icon} icon)` : ''} on top, `
}
...
function itemJa(it: Item): string {
  ...
  switch (it.kind) {
    ...
    case 'bottomNav': {
      const tabs = (it.tabs ?? []).map(
        t => `${q(t.label || 'ラベルなし')}(${t.icon || 'アイコンなし'})`,
      )
    ...
    case 'select': {
      ...
      const initial =
        it.selected !== undefined && it.tabs?.[it.selected]
          ? `、初期値は${q(it.tabs[it.selected].label)}`
          : '、初期値は未選択'
...
function itemEn(it: Item): string {
  ...
  switch (it.kind) {
    ...
    case 'select': {
      ...
      const initial =
        it.selected !== undefined && it.tabs?.[it.selected]
          ? `, initially ${q(it.tabs[it.selected].label)}`
          : ', initially none'
    ...
    case 'fabMenu': {
      const items = (it.tabs ?? []).map(
        t => `${q(t.label || 'unlabeled')} (${t.icon || 'no icon'})`,
      )
...
function itemZh(it: Item): string {
  ...
  switch (it.kind) {
    ...
    case 'select': {
      ...
      const initial =
        it.selected !== undefined && it.tabs?.[it.selected]
          ? `，初始值为${q(it.tabs[it.selected].label)}`
          : '，初始未选择'
...
function itemKo(it: Item): string {
  ...
  switch (it.kind) {
    ...
    case 'iconButton':
      return `${it.icon ?? '빈'} 아이콘의 ${v} 아이콘 버튼`
    case 'fab':
      return `${it.icon ?? '빈'} 아이콘의 ${v} FAB${it.size && it.size >= 96 ? '(대형)' : it.size && it.size <= 40 ? '(소형)' : ''}`
    case 'extendedFab':
      return `${q(it.label)}${it.icon ? ` 및 ${it.icon} 아이콘` : ''} 확장 FAB(${v})`
    case 'chip':
      return `${q(it.label)} 칩${it.checked ? '(선택됨)' : ''}${it.icon && !it.checked ? `(${it.icon} 아이콘 포함)` : ''}`
    case 'topAppBar':
      return `제목이 ${q(it.label)}인 상단 앱 바${it.icon ? `, 왼쪽 ${it.icon}` : ''}${it.icon2 ? `, 오른쪽 ${it.icon2}` : ''}${it.icon || it.icon2 ? ' 아이콘 버튼' : ''}`
    case 'bottomNav': {
      const tabs = (it.tabs ?? []).map(
        t => `${q(t.label || '레이블 없음')}(${t.icon || '아이콘 없음'})`,
      )
    ...
    case 'searchBar':
      return `자리표시자가 ${q(it.label)}인 검색창${it.icon2 ? `(오른쪽 끝에 ${it.icon2} 아이콘)` : ''}`
    case 'card': {
      const style =
        it.variant === 'elevated' ? '돌출' : it.variant === 'outlined' ? '윤곽선' : '채움'
    ...
    case 'listItem':
      return `${q(it.label)}${hasText(it.supporting) ? `(보조 텍스트 ${q(it.supporting!)})` : ''}${it.icon ? `, 앞쪽 ${it.icon} 아이콘${it.iconFill === 'none' ? '(배경 없음)' : it.iconFill ? `(배경 ${it.iconFill})` : ''}` : ''}${it.switch ? `, 끝에 스위치(초기 상태 ${it.checked ? '켜짐' : '꺼짐'})` : it.icon2 ? `, 뒤쪽 ${it.icon2}` : ''}${it.fill && it.fill !== 'surfaceContainerLow' ? `, 배경 ${it.fill}` : ''}`
    case 'dialog':
      return `제목 ${q(it.label)}${hasText(it.supporting) ? `, 본문 ${q(it.supporting!)}` : ''}${it.icon ? `, ${it.icon} 아이콘 포함` : ''} 대화상자(취소/확인 텍스트 버튼)`
    case 'snackbar':
      return `${q(it.label)} 스낵바${hasText(it.supporting) ? `(${q(it.supporting!)} 동작 포함)` : ''}`
    case 'textField':
      return `레이블이 ${q(it.label)}인 ${it.variant === 'filled' ? '채움' : '윤곽선'} 텍스트 입력란${it.icon ? `(앞쪽 ${it.icon} 아이콘)` : ''}${hasText(it.supporting) ? `. 보조 텍스트는 ${q(it.supporting!)}` : ''}`
    case 'select': {
      const opts = (it.tabs ?? []).map(t => q(t.label || '레이블 없음'))
      const initial =
        it.selected !== undefined && it.tabs?.[it.selected]
          ? `, 초깃값은 ${q(it.tabs[it.selected].label)}`
          : ', 초깃값은 없음'
    ...
    case 'switch':
      return `${q(it.label)} 스위치(초기 상태 ${it.checked ? '켜짐' : '꺼짐'}${it.noCheck ? ', 켜졌을 때 핸들에 체크 아이콘 없음' : ''})`
    case 'checkbox':
      return `${q(it.label)} 체크박스(초기 상태 ${it.checked ? '선택됨' : '선택 안 됨'})`
    case 'slider':
      return `슬라이더(초깃값 ${it.value ?? 40}%)`
    case 'text':
      return `${it.bold ? '굵은 ' : ''}텍스트 ${q(it.label)}(${it.size ?? 28}sp)`
    case 'image':
      return `${it.size ?? 200}dp 정사각형 이미지${imageSrc(it) ? `(${imageSrc(it)}의 이미지 표시)` : it.src ? '(지정한 이미지 표시)' : ' 자리표시자'}`
    case 'camera':
      return `${viewSize(it, 4 / 3)} 카메라 미리보기`
    case 'map':
      return `${viewSize(it, 3 / 4)} 지도`
    case 'divider':
      return '구분선'
    case 'box':
      return `${it.size ?? PHONE_W}×${it.size2 ?? 220}dp ${it.checked ? '하단 시트(위쪽 드래그 핸들 포함)' : '상자'}(배경 ${it.fill ?? 'surfaceContainerLow'}, ${boxCorners(it, 'ko')})`
    case 'loadingIndicator':
      return `M3 Expressive 형태 변환 로딩 표시기${it.contained ? '(컨테이너 포함)' : ''}`
    case 'linearProgress':
      return `${it.wavy ? '물결 모양 ' : ''}선형 진행 표시기(${it.value === undefined ? '불확정' : `${it.value}%`})`
    case 'circularProgress':
      return `${it.wavy ? '물결 모양 ' : ''}원형 진행 표시기(${it.value === undefined ? '불확정' : `${it.value}%`})`
    case 'splitButton':
      return `${q(it.label)}${it.icon ? `(${it.icon} 아이콘 포함)` : ''} ${v} 분할 버튼(오른쪽에 아래쪽 화살표가 있는 메뉴 영역)`
    case 'fabMenu': {
      const items = (it.tabs ?? []).map(
        t => `${q(t.label || '레이블 없음')}(${t.icon || '아이콘 없음'})`,
      )
    ...
    case 'radio':
      return `${q(it.label)} 라디오 버튼(초기 상태 ${it.checked ? '선택됨' : '선택 안 됨'})`
    case 'badge':
      return hasText(it.label) ? `${q(it.label)}을 표시하는 배지` : '작은 점 배지'
    default:
      return noun
  }
}
...
function boxCorners(it: Item, lang: Lang): string {
  ...
  if (each) {
    ...
    if (lang === 'ko')
      return `모서리 왼쪽 위 ${c.tl}dp / 오른쪽 위 ${c.tr}dp / 왼쪽 아래 ${c.bl}dp / 오른쪽 아래 ${c.br}dp`
...
function groupText(g: Group, lang: Lang): string {
  ...
  if (lang === 'ja') {
    if (kind === 'listItem')
      return `${g.items.length}項目のリスト。上から ${g.items.map(itemJa).join('、')}`
    if (kind === 'chip')
      return `${g.items.map(it => q(it.label) + (it.checked ? '(選択中)' : '')).join('')}のチップが横に並ぶチップグループ`
    if (kind === 'iconButton')
      return `${g.items.map(it => it.icon ?? '空').join('・')} のアイコンボタンが連結したボタングループ`
    const names = same
  ...
  if (lang === 'zh') {
    if (kind === 'listItem')
      return `${g.items.length}项的列表，从上到下依次为 ${g.items.map(itemZh).join('、')}`
    if (kind === 'chip')
      return `由${g.items.map(it => q(it.label) + (it.checked ? '(选中)' : '')).join('')}横向排列组成的标签片组`
    if (kind === 'iconButton')
      return `由 ${g.items.map(it => it.icon ?? '空').join('、')} 图标按钮相连组成的按钮组`
    const names = same
  ...
  if (lang === 'ko') {
    if (kind === 'listItem')
      return `${g.items.length}개 항목의 목록. 위에서부터 ${g.items.map(itemKo).join(', ')}`
    if (kind === 'chip')
      return `${g.items.map(it => q(it.label) + (it.checked ? '(선택됨)' : '')).join(', ')} 칩을 가로로 배치한 칩 그룹`
    if (kind === 'iconButton')
      return `${g.items.map(it => it.icon ?? '빈 아이콘').join(', ')} 아이콘 버튼을 연결한 버튼 그룹`
    const names = same
  ...
  if (kind === 'listItem')
    return `a list of ${g.items.length} items, top to bottom: ${g.items.map(itemEn).join('; ')}`
  if (kind === 'chip')
    return `a chip group: ${g.items.map(it => q(it.label) + (it.checked ? ' (selected)' : '')).join(', ')}`
  if (kind === 'iconButton')
    return `a connected group of icon buttons: ${g.items.map(it => it.icon ?? 'empty').join(', ')}`
  const names = same
      ...
      : lang === 'zh'
        ...
        : lang === 'ko'
          ...
          : it.checked
            ? 'ボトムシート'
            : 'ボックス'
  if (hasText(it.label) && it.kind !== 'text')
    return lang === 'en'
      ? `the ${q(it.label)} ${noun}`
      : `${q(it.label)}${lang === 'ko' ? ' ' : ''}${noun}`
...
function actionText(a: Action, frames: Frame[], lang: Lang): string | null {
  ...
  if (a.to === BACK_TARGET) {
    return lang === 'ja'
      ? '前の画面に戻る（入ったときの遷移を逆再生する）'
      : lang === 'zh'
        ? '返回上一个屏幕（反向播放进入时的过渡动画）'
        : lang === 'ko'
          ? '이전 화면으로 돌아간다(진입 전환을 반대로 재생)'
          : 'goes back to the previous screen (playing the entry transition in reverse)'
  }
...
function notes(g: Group, frames: Frame[], lang: Lang): string[] {
  ...
  for (const it of g.items) {
    ...
    if (it.toggle) {
      ...
      const label =
        it.toggle.label !== undefined && it.toggle.label !== it.label ? it.toggle.label : undefined
      if (lang === 'ja') {
        ...
        parts.push(
          `タップするたびにオン／オフが切り替わるトグルボタンにする${changes.length ? `（オンのときは${changes.join('、')}）` : ''}`,
        )
      } else if (lang === 'zh') {
        ...
        parts.push(
          `做成每次点击都切换开/关状态的切换按钮${changes.length ? `（开启时${changes.join('、')}）` : ''}`,
        )
      } else if (lang === 'ko') {
        ...
        parts.push(
          `탭할 때마다 켜짐/꺼짐이 전환되는 토글 버튼으로 만든다${changes.length ? `(켜졌을 때 ${changes.join(', ')})` : ''}`,
        )
      } else {
        ...
        parts.push(
          `is a toggle button that flips on / off with every tap${changes.length ? ` (when on, ${changes.join(' and ')})` : ''}`,
        )
      }
    }
...
const contains = (o: Rect, i: Rect, tol = 2) =>
  i.l >= o.l - tol && i.t >= o.t - tol && i.r <= o.r + tol && i.b <= o.b + tol
  ...
  nodes.forEach((n, i) => {
    ...
    for (let j = 0; j < i; j++) {
      ...
      if (
        contains(c.bb, n.bb) &&
        area(c.bb) > area(n.bb) &&
        (!parent || area(c.bb) < area(parent.bb))
      )
        parent = c
    }
...
function rowText(row: LNode[], where: string, lang: Lang, within: Rect): string {
  ...
  if (lang === 'ja') {
    const stretch = fillsRight
      ? `。最後の${groupName(last.g, 'ja')}は右端まで残りの幅いっぱいに伸ばします`
      : ''
  ...
  if (lang === 'ko') {
    const stretch = fillsRight
      ? `, 마지막 항목(${groupName(last.g, 'ko')})은 오른쪽 끝까지 남은 너비를 채웁니다`
      : ''
    return `${where}, 왼쪽부터 한 행에 다음 항목을 배치합니다: ${descs.join(', ')}(같은 줄에 세로 중앙 정렬하고 쌓거나 줄 바꿈하지 않음${stretch}).`
  }
  const stretch = fillsRight
    ? `; ${groupName(last.g, 'en')} stretches to fill the remaining width to the right edge`
    : ''
  ...
  rows.forEach((row, i) => {
    ...
    if (row.length === 1) {
      for (const other of nodes) {
        ...
        if (ov > 0 && ov >= area(first.bb) * 0.25 && !contains(other.bb, first.bb))
          overlaps.push(groupName(other.g, lang))
      }
    }
      ...
      line =
        lang === 'ja'
          ...
          : lang === 'zh'
            ...
            : lang === 'ko'
              ? `${line.replace(/\.$/, '')}(${o} 위에 일부 겹쳐 앞쪽에 그림).`
...
const RAIL_LEAD: Record<Lang, string> = {
  ja: '左端に',
  en: 'Along the left edge: ',
  zh: '左缘：',
  ko: '왼쪽 가장자리에 ',
}
...
function describeScreen(
  ...
  for (const g of rails)
    lines.push(
      `- ${RAIL_LEAD[lang]}${itemText(g.items[0], lang)}${lang === 'ja' || lang === 'zh' ? '。' : '.'}`,
    )
...
const STYLE_NOTES: Record<Lang, Partial<Record<Kind | 'boxSheet', string>>> = {
  ja: {
    ...
    searchBar:
      '検索バー: 高さ 56dp、角は完全な丸、背景は surfaceContainerHigh。先頭に検索アイコン、末尾に指定のアイコン。',
    ...
    dialog:
      'ダイアログ: 幅 312dp、角丸 28dp、背景は surfaceContainerHigh。見出しは headlineSmall、本文は bodyMedium、下部右寄せにテキストボタン。',
    snackbar:
      'スナックバー: 高さ 48dp、角丸 8dp、背景は inverseSurface、文字は inverseOnSurface。アクションは inversePrimary のテキストボタン。画面下部から 16dp 上に表示し、数秒で消える。',
    textField:
    ...
    switch:
      'スイッチ: M3 標準サイズ（トラック 52×32dp）。オンは primary、オフは surfaceContainerHighest に outline の枠。ラベルは左、スイッチは右端。',
    checkbox:
      'チェックボックス: 18dp の四角、角丸 2dp、チェック時は primary。ラベルは右に bodyLarge。',
    slider:
      'スライダー: M3 Expressive の太いトラック（高さ 16dp）と縦長のハンドル（幅 4dp・高さ 44dp）。ハンドルの左は primary、右は secondaryContainer。ドラッグで値を変えられる。',
    text: 'テキスト: 指定の sp サイズ。見出しは onSurface、説明文は onSurfaceVariant、行間はサイズの 1.3〜1.5 倍。タップしてもリップルなどの反応は付けない。',
    image:
      '画像: 角丸 20dp、指定がなければ surfaceContainerHighest のプレースホルダー。アスペクト比を保って中央でクロップ。',
    camera:
      'カメラプレビュー: 角丸 20dp。端末のカメラ映像をこの領域に表示し、権限が無い間は inverseSurface の暗い面にカメラアイコンを置く。',
    ...
    linearProgress:
      'リニアプログレス: M3 Expressive の太いバー。波形指定のときは進行中に波打つ wavy スタイルにする。トラックは secondaryContainer、進捗は primary。',
    circularProgress:
      'サーキュラープログレス: M3 Expressive の太いストローク。波形指定のときは波打つ wavy スタイルにする。',
    splitButton:
    ...
    radio:
      'ラジオボタン: 20dp の円。選択時は primary の枠と中央の点、未選択は onSurfaceVariant の枠。同じグループ内では 1 つだけ選べる。ラベルは右に bodyLarge。',
    badge:
      'バッジ: 文字なしは 6dp の点、文字ありは高さ 16dp のピル。背景は error、文字は onError の labelSmall。アイコンや項目の右上に重ねて置く。',
  },
  en: {
    button:
    ...
    searchBar:
      'Search bar: 56dp tall, fully rounded, on surfaceContainerHigh, with a leading search icon and the specified trailing icon.',
    ...
    dialog:
      'Dialogs: 312dp wide, 28dp corners, on surfaceContainerHigh. Headline in headlineSmall, body in bodyMedium, text buttons aligned right at the bottom.',
    snackbar:
      'Snackbar: 48dp tall, 8dp corners, inverseSurface background with inverseOnSurface text; the action is an inversePrimary text button. Show it 16dp above the bottom edge and dismiss after a few seconds.',
    textField:
    ...
    switch:
      'Switches: standard M3 size (52×32dp track). On is primary; off is surfaceContainerHighest with an outline border. Label on the left, switch at the trailing edge.',
    checkbox:
      'Checkboxes: 18dp square with 2dp corners, primary when checked, label on the right in bodyLarge.',
    slider:
    ...
    camera:
      'Camera preview: 20dp corners. Show the device camera feed in this area; while permission is missing, show a camera icon on a dark inverseSurface pane.',
    ...
    linearProgress:
      'Linear progress: the thick M3 Expressive bar; use the wavy style when specified. Track is secondaryContainer, progress is primary.',
    circularProgress:
      'Circular progress: the thick M3 Expressive stroke; use the wavy style when specified.',
    splitButton:
    ...
    radio:
      'Radio buttons: 20dp circles. Selected shows a primary ring with a center dot, unselected an onSurfaceVariant ring. Only one in a group can be selected. Label on the right in bodyLarge.',
    badge:
      'Badges: a 6dp dot without text, a 16dp-tall pill with text. Background error, text onError in labelSmall. Overlay it on the top-right of an icon or item.',
  },
  zh: {
    button:
    ...
    iconButton:
      '图标按钮：48dp 圆形。按指定使用填充／色调／描边／标准样式。相连的图标按钮组实现为 Connected button group。',
    ...
    searchBar:
      '搜索栏：高 56dp，完全圆角，背景为 surfaceContainerHigh。前置搜索图标，后置指定图标。',
    ...
    dialog:
      '对话框：宽 312dp，圆角 28dp，背景为 surfaceContainerHigh。标题用 headlineSmall，正文用 bodyMedium，底部右对齐放文字按钮。',
    snackbar:
      '消息条：高 48dp，圆角 8dp，背景为 inverseSurface，文字为 inverseOnSurface。操作为 inversePrimary 的文字按钮。显示在距屏幕底部 16dp 处，数秒后消失。',
    textField:
    ...
    slider:
      '滑块：M3 Expressive 的粗轨道（高 16dp）和竖长手柄（宽 4dp、高 44dp）。手柄左侧为 primary，右侧为 secondaryContainer。可拖动改变数值。',
    ...
    camera:
      '相机预览：圆角 20dp。在此区域显示设备相机画面；未获得权限时，在 inverseSurface 的深色面板上显示相机图标。',
    ...
    boxSheet:
      '容器框／底部面板：带指定背景色和圆角的容器。只有描述中带拖动条的才做成从底部滑出的模态底部面板（ModalBottomSheet），其余容器框只是普通的背景容器。',
    loadingIndicator:
      '加载指示：使用 M3 Expressive 形状变化的 LoadingIndicator（旋转并在多边形之间变形）。带容器的放在 secondaryContainer 的圆形中。',
    linearProgress:
      '线性进度条：M3 Expressive 的粗进度条。指定波浪形时使用进行中波动的 wavy 样式。轨道为 secondaryContainer，进度为 primary。',
    ...
    radio:
      '单选按钮：20dp 圆形。选中时为 primary 的圆环加中心圆点，未选中为 onSurfaceVariant 圆环。同一组内只能选一个。标签在右侧，用 bodyLarge。',
    badge:
      '徽标：无文字时为 6dp 圆点，有文字时为高 16dp 的胶囊。背景为 error，文字为 onError 的 labelSmall。叠放在图标或项目的右上角。',
  },
  ko: {
    button:
      '버튼: 높이 56dp의 중간 크기, 완전 둥근 알약 모양. 채움은 primary, 토널은 secondaryContainer, 윤곽선은 1dp outline 테두리를 사용한다. 연결 버튼 그룹은 간격 3dp, 맞닿는 안쪽 모서리 8dp, 바깥쪽 모서리는 둥글게 유지한다.',
    navRail:
      '내비게이션 레일: 너비 80dp, 배경 surfaceContainer, 왼쪽 가장자리의 전체 높이를 채운다. 항목은 위에서부터 세로로 배치한다. 선택 항목은 secondaryContainer 알약 표시기(56×32dp), 채운 아이콘과 아래쪽 labelMedium 레이블로 표시한다. 콘텐츠는 레일 오른쪽에 배치한다.',
    iconButton:
      '아이콘 버튼: 48dp 원형. 지정된 채움, 토널, 윤곽선, 표준 스타일을 사용하며 연결된 아이콘 버튼은 Connected button group으로 구현한다.',
    ...
    topAppBar:
      '상단 앱 바: 높이 64dp, 배경 surface. 상태 표시줄 뒤까지 배경을 늘리고 시스템 인셋만큼 위쪽 여백을 둔다. 제목은 titleLarge, 양쪽 아이콘 버튼은 48dp를 사용한다.',
    bottomNav:
      '내비게이션 바: 높이 80dp, 배경 surfaceContainer. 제스처 내비게이션 영역까지 배경을 늘리고 시스템 인셋만큼 아래쪽 여백을 둔다. 선택 항목은 64×32dp secondaryContainer 알약 표시기, 채운 아이콘, labelMedium 레이블로 표시한다.',
    searchBar:
      '검색창: 높이 56dp, 완전 둥근 모서리, 배경 surfaceContainerHigh. 앞쪽 검색 아이콘과 지정된 뒤쪽 아이콘을 둔다.',
    ...
    listItem:
      '목록 항목: 높이 72dp, 앞쪽 아이콘 24dp(별도 지정이 없으면 40dp primaryContainer 원 위), 주 텍스트 bodyLarge, 보조 텍스트 bodyMedium/onSurfaceVariant. 연결 목록은 간격 3dp, 바깥 모서리 28dp, 안쪽 모서리 8dp.',
    dialog:
      '대화상자: 너비 312dp, 모서리 28dp, 배경 surfaceContainerHigh. 제목 headlineSmall, 본문 bodyMedium, 텍스트 버튼은 아래쪽 오른쪽 정렬.',
    snackbar:
      '스낵바: 높이 48dp, 모서리 8dp, inverseSurface 배경과 inverseOnSurface 텍스트. 동작은 inversePrimary 텍스트 버튼으로 하고 아래쪽에서 16dp 띄워 몇 초 뒤 닫는다.',
    textField:
      '텍스트 입력란: 높이 56dp. 윤곽선형은 모서리 16dp와 outline 테두리, 채움형은 surfaceContainerHighest 배경과 밑줄을 사용한다. 포커스 시 레이블을 올리고 테두리를 2dp primary로 바꾼다.',
    select:
      '드롭다운: 텍스트 입력란과 같은 외관(높이 56dp, 윤곽선 또는 채움)에 끝에 arrow_drop_down 아이콘. Exposed dropdown menu로 구현하고, 탭하면 아래에 메뉴(surfaceContainer, 모서리 4dp, 항목 높이 48dp)를 열어 고른 값을 입력란에 표시한다.',
    switch:
      '스위치: M3 표준 크기(트랙 52×32dp). 켜짐은 primary, 꺼짐은 surfaceContainerHighest와 outline 테두리. 레이블은 왼쪽, 스위치는 오른쪽에 둔다.',
    checkbox:
      '체크박스: 18dp 사각형, 모서리 2dp, 선택 시 primary. 레이블은 오른쪽에 bodyLarge로 표시한다.',
    slider:
      '슬라이더: M3 Expressive의 두꺼운 16dp 트랙과 4×44dp 세로 핸들. 핸들 왼쪽은 primary, 오른쪽은 secondaryContainer이며 드래그로 값을 바꾼다.',
    text: '텍스트: 지정된 sp 크기. 제목은 onSurface, 설명은 onSurfaceVariant, 줄 높이는 글자 크기의 1.3~1.5배. 탭 반응은 넣지 않는다.',
    image:
      '이미지: 모서리 20dp. 이미지가 없으면 surfaceContainerHighest 자리표시자를 사용하고 비율을 유지해 가운데에서 자른다.',
    camera:
      '카메라 미리보기: 모서리 20dp. 이 영역에 기기 카메라 화면을 표시하고, 권한이 없는 동안은 inverseSurface의 어두운 면 위에 카메라 아이콘을 둔다.',
    ...
    boxSheet:
      '상자/하단 시트: 지정된 배경과 모서리를 가진 컨테이너. 드래그 핸들이 명시된 것만 아래에서 올라오는 ModalBottomSheet로 만들고 나머지는 단순 배경 컨테이너로 둔다.',
    loadingIndicator:
      '로딩: 다각형이 회전하며 형태가 바뀌는 M3 Expressive LoadingIndicator를 사용한다. 컨테이너형은 secondaryContainer 원 안에 둔다.',
    linearProgress:
      '선형 진행 표시기: M3 Expressive의 두꺼운 바. 지정된 경우 물결 스타일을 사용하며 트랙은 secondaryContainer, 진행은 primary로 표시한다.',
    circularProgress:
      '원형 진행 표시기: M3 Expressive의 두꺼운 스트로크. 지정된 경우 물결 스타일을 사용한다.',
    splitButton:
      '분할 버튼: M3 Expressive SplitButton. 왼쪽은 주 동작, 오른쪽 화살표 영역은 메뉴를 연다. 두 영역 간격 2dp, 바깥 모서리는 완전 둥글게, 안쪽은 8dp로 한다.',
    fabMenu:
      'FAB 메뉴: M3 Expressive FloatingActionButtonMenu. 닫혔을 때는 일반 FAB이고 탭하면 항목이 위로 차례로 나타나며 아이콘은 close로 바뀐다. 각 항목은 높이 56dp, 완전 둥근 모서리, 아이콘과 레이블을 포함한다.',
    toolbar:
      '플로팅 도구 모음: M3 Expressive HorizontalFloatingToolbar. 높이 64dp, 완전 둥근 모서리로 화면 아래쪽에서 16dp 띄운다. 표준은 surfaceContainer, 비브런트는 primaryContainer, 내부 아이콘 버튼은 48dp.',
    tabs: '탭: M3 기본 탭. 높이 48dp, 레이블 titleSmall. 선택 탭은 primary 텍스트와 레이블 너비의 3dp 표시기를 사용하고 아래에 outlineVariant 구분선을 둔다.',
    radio:
      '라디오 버튼: 20dp 원형. 선택 시 primary 테두리와 가운데 점, 미선택 시 onSurfaceVariant 테두리. 그룹에서 하나만 선택되며 레이블은 오른쪽 bodyLarge.',
    badge:
      '배지: 텍스트가 없으면 6dp 점, 있으면 높이 16dp 알약 모양. 배경 error, 텍스트 onError/labelSmall로 아이콘이나 항목 오른쪽 위에 겹쳐 둔다.',
  },
}
...
> = {
  ja: {
    shape: {
      square:
        '角丸は控えめにする: M3 の shape スケールを全体に小さく取り（ボタン・チップは 8〜12dp、カードや画像は 8dp、ダイアログは 12dp 程度）、ピル型は使わない。',
      rounded:
        '角丸は M3 Expressive の標準値のまま（ボタンはピル型、カードは 20dp、ダイアログは 28dp）。',
      full: '角丸は最大限に取る: ボタン・チップ・入力欄はピル型、カードや画像は 32dp、ダイアログやシートは 40dp 程度にする。',
    },
    emphasized:
      '見出し・ボタンラベル・タブは M3 Expressive の emphasized タイポグラフィ（headlineMediumEmphasized などの太めのウェイト）を使う。',
    plainType: 'タイポグラフィは M3 の標準ウェイト。',
    motion: {
      standard:
        'モーションは MotionScheme.standard()。画面遷移や状態変化は弾まない滑らかな動きにする。',
      expressive:
        'モーションは MotionScheme.expressive()。画面遷移や状態変化には軽く弾むスプリングを使う。',
    },
  },
  en: {
    shape: {
      square:
        'Keep corners modest: shrink the M3 shape scale throughout (buttons and chips 8–12dp, cards and images 8dp, dialogs about 12dp) and avoid pill shapes.',
      rounded:
    ...
    emphasized:
      'Headlines, button labels and tabs use the M3 Expressive emphasized typography (the heavier headlineMediumEmphasized and similar styles).',
    plainType: 'Typography uses the standard M3 weights.',
    motion: {
      standard:
        'Motion uses MotionScheme.standard(): smooth transitions and state changes with no bounce.',
      expressive:
  ...
  zh: {
    shape: {
      square:
        '圆角保持克制：整体缩小 M3 的形状比例（按钮和标签片 8〜12dp，卡片和图片 8dp，对话框约 12dp），不使用胶囊形。',
      rounded: '圆角沿用 M3 Expressive 的默认值（按钮为胶囊形，卡片 20dp，对话框 28dp）。',
      full: '圆角尽量放大：按钮、标签片和输入框为胶囊形，卡片和图片 32dp，对话框和面板约 40dp。',
    },
    emphasized:
      '标题、按钮文字和标签页使用 M3 Expressive 的 emphasized 字体样式（headlineMediumEmphasized 等更粗的字重）。',
  ...
  ko: {
    shape: {
      square:
        '모서리는 절제한다. 전체 M3 모양 배율을 줄이고(버튼과 칩 8~12dp, 카드와 이미지 8dp, 대화상자 약 12dp) 알약 모양은 사용하지 않는다.',
    ...
    emphasized:
      '제목, 버튼 레이블, 탭은 더 굵은 headlineMediumEmphasized 등 M3 Expressive 강조 글꼴 스타일을 사용한다.',
    plainType: 'M3 표준 글자 굵기를 사용한다.',
    motion: {
      standard:
        '모션은 MotionScheme.standard()를 사용한다. 화면 전환과 상태 변화는 튀지 않고 부드럽게 처리한다.',
      expressive:
        '모션은 MotionScheme.expressive()를 사용한다. 화면 전환과 상태 변화에 가볍게 튀는 스프링 효과를 적용한다.',
    },
  },
}
    ...
    font?.key === 'system'
      ? lang === 'ja'
        ...
        : lang === 'zh'
          ...
          : lang === 'ko'
            ? '기기의 시스템 글꼴'
            : "the device's system font"
      : (font?.label ?? 'Roboto')
  const sp = lang === 'en' || lang === 'ko' ? ' ' : ''
  return [
    `- ${n.shape[th.shape]}`,
    `- ${FONT_NOTE[lang](fontName)}${sp}${th.emphasized ? n.emphasized : n.plainType}`,
    `- ${n.motion[th.motion]}`,
  ]
}
...
const GENERAL: Record<Lang, (string | ((pl: Platform) => string))[]> = {
  ja: [
    ...
    (pl: Platform) =>
      `データは本物として扱う。ユーザーが作成したデータは${pl === 'web' ? 'ブラウザに（IndexedDB など）' : '端末に（Room や DataStore など）'}永続化し、${pl === 'web' ? '再読み込み' : '再起動'}後も残す。ダミーやサンプルのデータは入れず、何もない状態には空の案内を表示する。入力は検証し、失敗や削除は適切に確認・通知する。`,
    ...
    (pl: Platform) =>
      `コンポーネントは ${pl === 'web' ? 'Material Web' : 'Jetpack Compose の material3（Expressive API を含む最新版）'} の標準部品を使い、ライブラリにある部品を独自描画しない。`,
    ...
    (pl: Platform) =>
      `${pl === 'web' ? 'ブラウザでの' : 'エミュレータや実機での'}動作検証は不要。実装が終わったら${pl === 'web' ? 'production build を実行し、その出力' : '署名済みの release APK '}を成果物として提供する。`,
  ],
  en: [
    ...
    (pl: Platform) =>
      `Use the standard components from ${pl === 'web' ? 'Material Web' : 'Jetpack Compose material3 (latest, including the Expressive APIs)'}; do not custom-draw parts the library provides.`,
    ...
    (pl: Platform) =>
      `Do not verify ${pl === 'web' ? 'in a browser' : 'on an emulator or a device'}. When the implementation is done, ${pl === 'web' ? 'run the production build and provide its output' : 'produce a signed release APK'} as the deliverable.`,
  ],
  zh: [
    '先根据屏幕目的判断这是什么类型的应用，并实现该类应用通常应有的功能（新建、列表、详情、编辑、删除、搜索、设置等，视情况而定），即使草图中没有画出。',
    (pl: Platform) =>
      `把数据当作真实数据处理：用户创建的数据要持久化到${pl === 'web' ? '浏览器（IndexedDB 等），重新加载' : '设备（Room、DataStore 等），重启'}后仍保留。不要放入虚拟或示例数据，没有数据时显示空状态提示。校验输入，删除和失败要有适当的确认或提示。`,
    ...
    (pl: Platform) =>
      `组件使用 ${pl === 'web' ? 'Material Web' : 'Jetpack Compose material3（包含 Expressive API 的最新版）'} 的标准组件，库里已有的组件不要自行绘制。`,
    ...
    (pl: Platform) =>
      `不需要在${pl === 'web' ? '浏览器' : '模拟器或真机'}上验证。实现完成后${pl === 'web' ? '运行 production build 并提供其输出' : '生成已签名的 release APK '}作为交付物。`,
  ],
  ko: [
    '화면의 목적에서 앱의 종류를 판단하고, 스케치에 없더라도 그 종류의 앱에 일반적으로 필요한 기능(만들기, 목록, 상세, 편집, 삭제, 검색, 설정 등)을 구현한다.',
    (pl: Platform) =>
      `데이터를 실제 데이터로 취급한다. 사용자가 만든 데이터는 ${pl === 'web' ? '브라우저(IndexedDB 등)에 저장해 새로고침' : '기기(Room, DataStore 등)에 저장해 재시작'} 후에도 유지한다. 더미나 샘플 데이터는 넣지 않고 데이터가 없으면 빈 상태를 표시한다. 입력을 검증하고 실패와 삭제는 적절히 확인하거나 알린다.`,
    ...
    (pl: Platform) =>
      `${pl === 'web' ? 'Material Web' : '최신 Expressive API를 포함한 Jetpack Compose material3'}의 표준 컴포넌트를 사용하고 라이브러리에 있는 부품을 직접 그리지 않는다.`,
    ...
    (pl: Platform) =>
      `${pl === 'web' ? '브라우저' : '에뮬레이터나 실제 기기'} 동작 검증은 필요 없다. 구현 후 ${pl === 'web' ? 'production build를 실행하고 그 출력' : '서명된 release APK'}을 결과물로 제공한다.`,
  ],
}
...
const STYLE_NOTES_WEB: Record<Lang, Partial<Record<Kind, string>>> = {
  ko: {
    topAppBar:
      '상단 앱 바: 높이 64dp, 배경 surface. 제목은 titleLarge, 양쪽 아이콘 버튼은 48dp를 사용한다. 스크롤 시 surfaceContainer로 색상이 바뀌는 표준 동작을 사용한다.',
    bottomNav:
      '내비게이션 바: 높이 80dp, 배경 surfaceContainer. 선택 항목은 secondaryContainer 알약 표시기(64×32dp), 채운 아이콘과 labelMedium 레이블로 표시한다.',
  },
  ja: {
    topAppBar:
  ...
  zh: {
    topAppBar:
      '顶部应用栏：高 64dp，背景为 surface。标题用 titleLarge，左右图标按钮 48dp。滚动时变为 surfaceContainer 的标准行为即可。',
    bottomNav:
      '导航栏：高 80dp，背景为 surfaceContainer。选中项用 secondaryContainer 的胶囊指示器（宽 64dp、高 32dp）表示，图标为填充样式，标签用 labelMedium。',
  },
}
...
const PH = {
  ja: {
    screen: '画面',
    intro: (title: string, brief: string) =>
      `${title}を Material 3 Expressive のデザインで実装してください。${brief ? trimEnd(brief) + '。' : ''}`,
    ...
    platform: (pl: Platform) =>
      pl === 'web'
        ? '実装先は Web（ブラウザで動くアプリ）です。'
        : '実装先は Android（ネイティブアプリ）です。',
    ...
    placement: (place: Place) =>
      place === 'center'
        ? '本文の部品は画面の縦中央にまとめて配置します。'
        : place === 'bottom'
          ? '本文の部品は画面の下側（ナビゲーションバーの上）に寄せて配置します。'
          : '本文の部品は画面の高さいっぱいに均等な間隔で配置します（1 行だけなら縦中央）。',
    screenHead: (name: string, bg: string | undefined, has: boolean, size?: string) =>
      `${name}画面${size || bg ? `（${[size, bg ? `背景は ${bg}` : ''].filter(Boolean).join('、')}）` : ''}${has ? 'は上から順に次の通りです。重なっている部品はその旨を書いています。' : 'はまだ空です。'}`,
    loose: '画面の外に置かれている部品（共通パーツや参考）:',
    freeform: '画面を上から順に説明します。',
    hBehavior: '## 振る舞いと画面遷移',
    hStyle: '## 各部品のスタイル',
    styleIntro:
      '使っている部品ごとの目安です。数値は M3 Expressive の標準値なので、標準コンポーネントで実現できるものは標準に任せ、内容に合わせて調整して構いません。',
    hGeneral: '## 全体の指針',
  },
  en: {
    screen: 'screen',
    intro: (title: string, brief: string) =>
      `Please implement ${title} in the Material 3 Expressive design language.${brief ? ` ${trimEnd(brief)}.` : ''}`,
    ...
    platform: (pl: Platform) =>
      pl === 'web'
        ? 'Build it for the web, as an app that runs in the browser.'
    ...
    placement: (place: Place) =>
      place === 'center'
        ? 'The body parts sit together in the vertical center of the screen.'
        : place === 'bottom'
          ? 'The body parts sit toward the bottom of the screen, above the navigation bar.'
          : 'The body parts are spread over the screen height with equal gaps (a single row sits in the vertical center).',
    screenHead: (name: string, bg: string | undefined, has: boolean, size?: string) =>
      `The ${name} screen${size || bg ? ` (${[size, bg ? `background ${bg}` : ''].filter(Boolean).join(', ')})` : ''}${has ? ', from top to bottom (overlapping parts are called out as such):' : ' is still empty.'}`,
  ...
  zh: {
    screen: '屏幕',
    intro: (title: string, brief: string) =>
      `请用 Material 3 Expressive 的设计实现${title}。${brief ? trimEnd(brief) + '。' : ''}`,
    ...
    placement: (place: Place) =>
      place === 'center'
        ...
        : place === 'bottom'
          ? '正文组件靠屏幕底部（导航栏上方）放置。'
          : '正文组件在屏幕高度上等距分布（只有一行时垂直居中）。',
    screenHead: (name: string, bg: string | undefined, has: boolean, size?: string) =>
      `${name}屏幕${size || bg ? `（${[size, bg ? `背景为 ${bg}` : ''].filter(Boolean).join('，')}）` : ''}${has ? '从上到下依次如下（重叠的组件会特别说明）：' : '目前为空。'}`,
  ...
  ko: {
    screen: '화면',
    intro: (title: string, brief: string) =>
      `Material 3 Expressive 디자인으로 구현해 주세요: ${title}.${brief ? ` ${trimEnd(brief)}.` : ''}`,
    titleOnly: (name: string) => `${name} 화면`,
    titleAll: (n: number) => (n > 1 ? '이 앱' : '이 화면'),
    target: (vp: Viewport, pl: Platform, dark: boolean, both: boolean) =>
      `${vp === 'phone' ? '세로형 휴대전화 화면(412×892dp)을 대상으로 하며' : vp === 'desktop' ? `${pl === 'web' ? '데스크톱 브라우저' : '가로형 태블릿'} 화면(1280×800 기준)을 대상으로 하며` : vp === 'mixed' ? `세로형 휴대전화(412×892)와 ${pl === 'web' ? '데스크톱 브라우저' : '가로형 태블릿'}(1280×800)을 모두 지원하며, 이름이 같은 화면은 서로 다른 너비의 동일한 화면이므로 반응형으로 구현하고` : '레이아웃은 자유 배치이며'}, ${both ? '라이트 모드와 다크 모드를 모두 지원하고 기기의 시스템 설정을 따른다' : `${dark ? '다크' : '라이트'} 모드만 지원한다`}.`,
    platform: (pl: Platform) =>
      pl === 'web'
        ? '브라우저에서 실행되는 웹 앱으로 구현한다.'
        : 'Android 네이티브 앱으로 구현한다.',
    schemeHead: (dark: boolean) => (dark ? '다크 색상 구성:' : '라이트 색상 구성:'),
    sketch:
      '아래 화면 구성은 의도를 전달하는 대략적인 스케치이며 완성 사양이 아니다. 정적인 그림처럼 복제하지 말고 이 종류의 제품에 일반적으로 필요한 기능을 갖춘 실제 사용 가능한 앱으로 완성한다.',
    hColor: '## 색상',
    dynamic: (pl: Platform) =>
      pl === 'web'
        ? '동적 색상을 사용한다. 브라우저나 운영체제에서 사용자의 강조 색상을 제공하면 이를 기준으로 Material 3 색상 구성을 생성하고, 사용할 수 없으면 아래 색상으로 대체한다.'
        : '동적 색상을 사용한다. Android 12 이상에서는 사용자 배경화면에서 생성된 색상 구성(dynamicLightColorScheme / dynamicDarkColorScheme)을 적용하고 사용할 수 없는 기기에서는 아래 색상을 대체 값으로 사용한다.',
    colorIntro: (label: string, fallback: boolean, th: Theme) => {
    ...
    placement: (place: Place) =>
      place === 'center'
        ? '본문 부품은 화면 세로 가운데에 모아 배치한다.'
        : place === 'bottom'
          ? '본문 부품은 화면 아래쪽(내비게이션 바 위)에 붙여 배치한다.'
          : '본문 부품은 화면 높이에 걸쳐 같은 간격으로 배치한다(한 줄뿐이면 세로 가운데).',
    screenHead: (name: string, bg: string | undefined, has: boolean, size?: string) =>
      `${name} 화면${size || bg ? `(${[size, bg ? `배경 ${bg}` : ''].filter(Boolean).join(', ')})` : ''}. ${has ? '위에서부터 다음과 같습니다. 겹친 부품은 별도로 표시합니다.' : '아직 비어 있습니다.'}`,
    loose: '화면 밖에 놓인 부품(공통 부품 또는 참고):',
    freeform: '화면을 위에서부터 설명합니다.',
    hBehavior: '## 동작 및 화면 전환',
    hStyle: '## 부품별 스타일',
    styleIntro:
      '사용된 부품별 지침입니다. 수치는 M3 Expressive 기본값이며 표준 컴포넌트가 제공하는 동작은 그대로 사용하고 내용에 맞게 조정할 수 있습니다.',
    hGeneral: '## 전체 지침',
  },
}
...
export function buildPrompt(
  ...
  const styleNotes = kindsUsed
    .map(k =>
      k === 'box' && sheet
        ? STYLE_NOTES[lang].boxSheet
        : (platform === 'web' && STYLE_NOTES_WEB[lang][k]) || STYLE_NOTES[lang][k],
    )
    .filter((s): s is string => !!s)

  const title = only
    ? ph.titleOnly(q(only.name || ph.screen))
    : doc.title.trim() || ph.titleAll(frames.length)
  ...
  if (groups.length === 0) {
    ...
    frames.forEach((f, i) => {
      ...
      if (hasText(f.note))
        lines.push(
          lang === 'ja' || lang === 'zh' ? `${trimEnd(f.note!)}。` : `${trimEnd(f.note!)}.`,
        )
      lines.push(
        ph.screenHead(
          q(f.name || ph.screen),
          f.bg && f.bg !== 'surface' ? f.bg : undefined,
          gs.length > 0,
          sizeLabel(f, viewport, lang),
        ),
      )
  ...
  const behavior = [
    ...groups.flatMap(g => notes(g, allFrames, lang)),
    ...frames.flatMap(f => swipeNotes(f, allFrames, lang)),
  ]
  if (behavior.length) {
    lines.push('')
