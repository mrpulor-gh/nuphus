    ...
    groups: snap.groups.map(group => ({
      ...
      items: group.items.map(item => ({
        ...
        ...(item.supporting !== undefined && {
          supporting: translateDefaultText(item.supporting, item.kind, 'supporting', lang),
        }),
        ...(item.tabs && {
...
const LEFT_TABS: {
  key: LeftTab
  icon: string
  title: 'parts' | 'layers' | 'colors' | 'shape' | 'typography' | 'motion' | 'ai'
}[] = [
...
export function UiPrototypeCanvas() {
  ...
  const changeLanguage = (next: Lang) => {
    ...
    const translated = translateSnapshot(
      { groups: groupsRef.current, frames: framesRef.current },
      next,
    )
    const tidy = tidyRef.current
    if (tidy) {
      tidyRef.current =
        tidy.after === groupsRef.current
          ? {
              ...tidy,
              before: translateSnapshot({ groups: tidy.before, frames: framesRef.current }, next)
                .groups,
  ...
  const inBin = (clientX: number) =>
    !mobileRef.current &&
    clientX >= 0 &&
    clientX <= (leftOpenRef.current ? leftWRef.current : RAIL_W)
  ...
  const fit = useCallback(() => {
    ...
    const z = clamp(
      Math.min((r.width - pad * 2) / (x1 - x0), (r.height - top - bottom) / (y1 - y0), 1),
      MIN_Z,
      MAX_Z,
    )
    setView({
  ...
  useEffect(() => {
    ...
    const onWheel = (e: WheelEvent) => {
      ...
      if (e.ctrlKey || e.metaKey) {
        setZoomAt(viewRef.current.z * Math.exp(-e.deltaY * 0.0022), e.clientX, e.clientY)
      } else {
  ...
  const findSnap = useCallback(
    (item: Item, left: number, top: number): Snap | null => {
      ...
      for (const g of groupsRef.current) {
        if (g.free || g.axis !== spec.axis || !g.items[0] || !canJoin(g.items[0], item)) continue
    ...
    (item: Item, left: number, top: number): Guide | null => {
      ...
      if (frameRef.current === 'phone') {
        for (const f of framesRef.current) {
          ...
          xs.push(f.x, f.x + FRAME_MARGIN, f.x + w / 2, f.x + w - FRAME_MARGIN, f.x + w)
          ys.push(f.y, f.y + FRAME_MARGIN, f.y + h / 2, f.y + h - FRAME_MARGIN, f.y + h)
        }
      }
      const mine = (pos: number, len: number) => [pos, pos + len / 2, pos + len]
      let best: Guide = {}
      let bx = tol
      for (const c of xs)
  ...
  const onItemPointerDown = (e: React.PointerEvent, g: Group, index: number, item: Item) => {
    ...
    if (g.free) {
      setSelectedIds(cur =>
        e.shiftKey
          ? [...cur.filter(x => !g.items.some(it => it.id === x)), ...g.items.map(it => it.id)]
    ...
    sx.jump(left)
    sy.jump(top)
    setSelectedIds(cur => (e.shiftKey ? [...cur.filter(x => x !== item.id), item.id] : [item.id]))
    setSelectedFrameId(null)
    setSelectedLinkId(null)
    setRightTab('edit')
  ...
  useEffect(() => {
    ...
    const move = (e: PointerEvent) => {
      ...
      if (!d.active) {
        if (Math.hypot(pt.x - d.startX, pt.y - d.startY) * viewRef.current.z < 5) {
          setDrag({ ...d })
          return
        }
        d.active = true
      ...
      d.snap = d.overBin ? null : findSnap(d.item, pt.x - d.offX, pt.y - d.offY)
      d.guide = d.overBin || d.snap ? null : findGuide(d.item, pt.x - d.offX, pt.y - d.offY)
      setDrag({ ...d })
    }
    ...
    const up = () => {
      ...
      if (d.snap) {
        ...
        const commit = () => {
          setGroups(prev => {
            if (prev.some(g => g.items.some(it => it.id === item.id))) return prev
            return prev.map(g => {
              ...
              return {
                ...
                items: [...g.items.slice(0, t.index), item, ...g.items.slice(t.index)],
              }
            })
          })
          setDrag(null)
      ...
      if (d.fromPalette && (screenL < 0 || screenT < 0 || screenR > cw || screenB > ch)) {
        setSelectedIds(cur => cur.filter(x => x !== item.id))
        setDrag(null)
        return
      }
      ...
      const slot = targetFrame
        ? barSlotOf(groupsRef.current, targetFrame, framesRef.current, widthsRef.current)
        : null
        ...
        targetFrame && slot
          ? isBar
            ? carryItemSize(
                item,
                { w: PHONE_W, h: PHONE_H },
                { w: slot.w, h: frameSizeOf(targetFrame).h },
              )
            : fitHeight(item, frameSizeOf(targetFrame).h)
          : item
      ...
      const dropped: Group = {
        ...
        x:
          isBar && slot
            ? Math.round(slot.x)
            : d.guide?.gx !== undefined
              ? Math.round(rawX)
              : onGrid(rawX, origin.x),
      ...
      const ng =
        pulled === dropped
          ...
          : {
              ...pulled,
              x: d.guide?.gx !== undefined ? pulled.x : onGrid(pulled.x, origin.x),
              y: d.guide?.gy !== undefined ? pulled.y : onGrid(pulled.y, origin.y),
            }
      setGroups(prev =>
        prev.some(g => g.items.some(it => it.id === item.id)) ? prev : [...prev, ng],
      )
      setDrag(null)
    }
  ...
  const onFramePointerDown = (e: React.PointerEvent, f: Frame) => {
    ...
    const carried = groupsRef.current
      .filter(g => frameOfGroup(g, framesRef.current, widthsRef.current)?.id === f.id)
  ...
  useEffect(() => {
    ...
    const move = (e: PointerEvent) => {
      ...
      if (g.kind === 'frame') {
        ...
        setFrames(fs =>
          fs.map(f =>
            f.id === g.id ? { ...f, x: Math.round(g.fx + dx), y: Math.round(g.fy + dy) } : f,
          ),
        )
        setGroups(gs =>
          gs.map(gr => {
            const o = ids.get(gr.id)
            return o ? { ...gr, x: Math.round(o.x + dx), y: Math.round(o.y + dy) } : gr
          }),
        )
        return
      }
      ...
      g.x1 = pt.x
      g.y1 = pt.y
      if (!g.moved && Math.hypot(pt.x - g.x0, pt.y - g.y0) * viewRef.current.z > 4) g.moved = true
      if (g.moved) {
  ...
  const pasteClipboard = useCallback(() => {
    ...
    const pos: Record<string, { x: number; y: number }> | undefined = clip.group.pos
      ? {}
      : undefined
    if (pos)
      for (const it of clip.group.items)
        pos[idMap.get(it.id)!] = clip.group.pos?.[it.id] ?? { x: 0, y: 0 }
    const copy: Group = {
  ...
  const alignSelected = useCallback(
    (kind: AlignKind) => {
      ...
      const horizontal =
        kind === 'left' || kind === 'centerH' || kind === 'right' || kind === 'distributeH'
      ...
      if (distributing) {
        const sorted = [...units].sort((a, b) =>
          horizontal
            ? rects.get(a.id)!.l - rects.get(b.id)!.l
            : rects.get(a.id)!.t - rects.get(b.id)!.t,
        )
        const sizes = sorted.map(g =>
          horizontal
            ? rects.get(g.id)!.r - rects.get(g.id)!.l
            : rects.get(g.id)!.b - rects.get(g.id)!.t,
        )
        ...
        sorted.forEach((g, i) => {
          ...
          shift.set(
            g.id,
            horizontal
              ? { dx: Math.round(pos) - r.l, dy: 0 }
              : { dx: 0, dy: Math.round(pos) - r.t },
          )
          pos += sizes[i] + gap
        })
      } else {
        ...
        const others = all
          .filter(
            g =>
              !ids.has(g.items[0].id) &&
              !g.items.some(it => ids.has(it.id)) &&
              (!screenId || frameOfGroup(g, framesRef.current, widthsRef.current)?.id === screenId),
          )
          .map(g => rects.get(g.id)!)
        const hits = (r: { l: number; t: number; r: number; b: number }) =>
          others.filter(o => o.l < r.r && o.r > r.l && o.t < r.b && o.b > r.t)
        /* stepping away from the aligned edge: right of a left edge, up from a bottom edge; a centre tries both ways */
        const dir =
          kind === 'left' || kind === 'top' ? 1 : kind === 'right' || kind === 'bottom' ? -1 : 0
        for (const g of units) {
          ...
          for (let tries = 0, sign = dir || 1; tries < 12; tries++, sign = dir || -sign) {
            ...
            const step =
              8 +
              (horizontal
                ? Math.max(...blocking.map(o => o.r - o.l))
                : Math.max(...blocking.map(o => o.b - o.t)))
            if (horizontal)
              x = clamp(x + sign * step * (dir ? 1 : tries + 1), bb.l, Math.max(bb.l, bb.r - w))
  ...
  const nudge = useCallback(
    (dx: number, dy: number) => {
      if (selectedIds.length === 0 && selectedFrameId) {
        ...
        setFrames(fs => fs.map(x => (x.id === f.id ? { ...x, x: x.x + dx, y: x.y + dy } : x)))
        setGroups(gs => gs.map(g => (carried.has(g.id) ? { ...g, x: g.x + dx, y: g.y + dy } : g)))
        return
      }
      if (selectedIds.length === 0) return
      const ids = new Set(selectedIds)
  ...
  const clearAll = () => {
    ...
    if (groupsRef.current.length === 0 && framesRef.current.length === 0) return
    setDraftBefore(null)
    setQuickUndo(false)
    try {
  ...
  const aiReady =
    hasKey(aiSettings) && aiSettings.model.trim().length > 0 && isSecureUrl(aiSettings.baseUrl)
  const aiReason = !aiReady
    ? t('aiNoKey', lang)
    : !tidyTarget
      ? t('aiSelectScreen', lang)
      : undefined
  ...
  const runAi = async (action: AiActionKey, f: Frame, itemId?: string) => {
    ...
    try {
      if (action === 'describe') {
        const r = await proposeDescription(
          aiSettings,
          curDoc,
          widthsRef.current,
          f,
          ...
          fs.map(x =>
            x.id === f.id
              ? {
                  ...x,
                  note: r.note,
                  noteHistory: pushHistory(x.noteHistory, x.note),
                  name: r.name ?? x.name,
                }
              : x,
          ),
      ...
      if (!itemId) return
      const note = await proposeBehavior(
        aiSettings,
        curDoc,
        widthsRef.current,
        f,
          ...
          g.items.some(it => it.id === itemId)
            ? {
                ...
                items: g.items.map(it =>
                  it.id === itemId
                    ? { ...it, note, noteHistory: pushHistory(it.noteHistory, it.note) }
                    : it,
                ),
              }
            : g,
  ...
  const deleteFrame = useCallback(
    (id: string) => {
            ...
            items: g.items.map(it => {
              ...
              if (next.actions) {
                const actions = Object.fromEntries(
                  Object.entries(next.actions).filter(([, a]) => a.to !== id),
                )
      ...
      setSelectedIds(cur =>
        cur.filter(
          x => !groupsRef.current.some(g => gone.has(g.id) && g.items.some(it => it.id === x)),
        ),
      )
    },
    [snapshot],
          ...
          g.free ? (
            (corners =>
              layoutOf(g, widths).map(pl => (
                <div
                  key={pl.item.id}
                  style={{ position: 'absolute', left: pl.x - f.x, top: pl.y - f.y }}
                >
                  <M3Static
                    item={pl.item}
                    palette={p}
                    ...
                    style={
                      MEASURED.includes(pl.item.kind) ? undefined : { width: pl.w, height: pl.h }
                    }
                  />
                </div>
              )))(freeRadii(g, widths))
              ...
              {g.items.map((it, i) => {
                ...
                return (
                  <M3Static
                    ...
                    radii={radii}
                    style={
                      MEASURED.includes(it.kind)
                        ? undefined
                        : { width: sizeOf(it, widths).w, height: sizeOf(it, widths).h }
                    }
                  />
                )
              })}
    ...
    queueMicrotask(() => {
      ...
      if (f && r) {
        ...
        const z = clamp(
          Math.min(
            1.4,
            (window.innerHeight - 32) / maxH,
            (window.innerWidth - (wide ? 236 : 16)) / maxW,
          ),
          MIN_Z,
          MAX_Z,
        )
  ...
  useEffect(() => {
    const onKey = (e: KeyboardEvent) => {
      ...
      if (e.key === 'Delete' || e.key === 'Backspace') {
        e.preventDefault()
        if (selectedIds.length === 0 && selectedFrameId) deleteFrame(selectedFrameId)
        else deleteSelected()
        return
      }
      if (e.key === 'Escape') {
  ...
  const runRadii = (
    ...
    return axis === 'x' ? { tl: s, bl: s, tr: e, br: e } : { tl: s, tr: s, bl: e, br: e }
  }
  ...
  const renderGroup = (g: Group, ox: number, oy: number) => {
      ...
      return (
        ...
        >
          {layoutOf(g, widths).map(pl => (
            <div
              key={pl.item.id}
              style={{ position: 'absolute', left: pl.x - g.x, top: pl.y - g.y }}
            >
              <M3Node
                item={pl.item}
                palette={p}
    ...
    for (let i = 0; i <= g.items.length; i++) {
      ...
      if (i < g.items.length) cells.push({ ph: false, item: g.items[i], index: i })
    }
          ...
          if (c.ph) {
            return (
              <motion.div
                ...
                animate={g.axis === 'x' ? { width: phMain } : { height: phMain }}
                transition={OPEN}
                style={{
                  flex: '0 0 auto',
              ...
              .map(it => (
                <div
                  ...
                  style={{
                    ...
                    border:
                      it.variant === 'outlined' &&
                      (it.kind === 'button' || it.kind === 'chip' || it.kind === 'extendedFab')
                  ...
                  <div style={{ flex: 1, minHeight: 0 }}>
                    {leftTab === 'parts' ? (
                      <PartsPalette
                        ...
                        overBin={
                          (!!drag?.active && drag.overBin) ||
                          (gesture?.kind === 'group' && gesture.overBin)
                        }
                      />
                    ) : leftTab === 'color' ? (
                      <ColorPanel
                      ...
                      <LayersPanel
                        ...
                        onSelect={(ids, add) => {
                          setSelectedIds(cur =>
                            add ? [...cur.filter(x => !ids.includes(x)), ...ids] : ids,
                          )
                          setSelectedFrameId(null)
                          setSelectedLinkId(null)
                          setRightTab('edit')
          ...
          >
            ...
            >
              ...
              >
                    ...
                    return (
                      <div
                        ...
                        style={{
                          ...
                          top: f.y,
                          transition: easing
                            ? `left ${SETTLE_MS}ms cubic-bezier(0.2, 0, 0, 1)`
                            : undefined,
                        }}
                      >
                        <div
                        ...
                        <div
                          ...
                          style={{
                            ...
                            animation: draftBusy
                              ? 'nuphus-drift 3s ease-in-out infinite'
                              : undefined,
                            boxShadow: on
                        ...
                        >
                          <AnimatePresence>
                            {aiFrameId === f.id && <ThinkingRing key="ring" p={p} frame={f} />}
                          </AnimatePresence>
                          <div
                            data-screen={f.id}
                            style={{
                          ...
                          >
                            ...
                            {draftBusy && (
                              ...
                              >
                                <LoadingIndicator size={96} color="url(#nuphus-drafting)" />
                              </div>
                            )}
                          </div>
                        </div>
                ...
                {groups.filter(g => !frameOf.has(g.id)).map(g => renderGroup(g, 0, 0))}
            ...
            {draftBusy && (
              ...
              >
                <svg width={0} height={0} style={{ position: 'absolute' }} aria-hidden>
                  <defs>
                    <linearGradient id="nuphus-drafting" x1="0" y1="0" x2="1" y2="1">
            ...
            {!rightOpen && !isMobile && (
              <div style={{ position: 'absolute', right: 20, top: 20, zIndex: 45 }}>
                <IconBtn
          ...
          >
            ...
            >
              ...
              <h1
                ...
                style={{ margin: '16px 0 8px', fontSize: 22, lineHeight: 1.25 }}
              >
