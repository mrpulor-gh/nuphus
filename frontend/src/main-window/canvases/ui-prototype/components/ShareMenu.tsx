...
export function ShareDialog({
  ...
  const copyAsk = async () => {
    const text = t('askAiText', lang)
      .replace('{url}', guideUrl())
      .replace('{idea}', idea.trim() || t('askAiIdeaFallback', lang))
    try {
  ...
  const pill = (
    ...
    label: string,
    onClick: () => void,
    opts?: { primary?: boolean; disabled?: boolean; corners?: 'left' | 'right'; title?: string },
  ) => {
    const outer = 20
    const inner = 8
    const radius =
          ...
          >
            <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
              ...
              {pill(
                ...
                copied === 'link' ? t('copied', lang) : t('shareLinkCopy', lang),
                copyLink,
                { title: t('shareLinkHint', lang) },
              )}
            </div>
            <p style={{ margin: 0, fontSize: 13, lineHeight: 1.55, color: p.onSurfaceVariant }}>
              {t('askAiHint', lang)}
            </p>
            <textarea
              value={idea}
            ...
            <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
              ...
              <span
                style={{
                  ...
                  gap: 6,
                  fontSize: 13,
                  fontWeight: copied === 'ask' ? 600 : 400,
                  color: copied === 'ask' ? p.primary : p.onSurfaceVariant,
                }}
              >
                {copied === 'ask' ? (
                  <>
              ...
              <div style={{ display: 'inline-flex', gap: 3, flex: '0 0 auto' }}>
                {pill(
                  copied === 'ask' ? 'check' : 'content_copy',
                  copied === 'ask' ? t('copied', lang) : t('askAiCopy', lang),
                  copyAsk,
                  { corners: 'left', title: t('askAiCopyTitle', lang) },
                )}
                {aiReady
