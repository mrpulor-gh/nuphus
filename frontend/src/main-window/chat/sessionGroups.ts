 ...
 *    不得落进「未分组」（决策 8）；
 * 4. 组内按 `updated_at` 倒序（最近对话在前），同值保持后端顺序（稳定）；
 ...
 * @param projects         可见项目文件夹（书签 → auto），顺序即组顺序
...
export function buildSessionGroups<T extends GroupSessionLike>(
  ...
  for (const g of groups) g.sessions = sortByUpdatedDesc(g.sessions)
  if (ungrouped.length > 0) {
    groups.push({
      ...
      isCurrent: false,
      auto: false,
      sessions: sortByUpdatedDesc(ungrouped),
    })
  }
  return groups
}
