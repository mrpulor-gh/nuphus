...
export const initialApiHealthState = (): ApiHealthState => ({ status: 'unknown', stableSince: null, lastTransitionAt: Date.now(), currentTurnId: 0, consecutiveFailures: 0, retryCount: 0, incidents: [], unreadCount: 0, pulse: null })
...
export function ApiSignalIcon({ status, size = 13 }: { status: ApiHealthStatus; size?: number }) {
  return (
    <svg
      ...
      aria-hidden="true"
    >
      <circle className="api-health-halo" cx="8" cy="8" r="6.5" stroke="currentColor" strokeWidth="1.5" strokeOpacity="0.35" />
      <circle className="api-health-core" cx="8" cy="8" r="3.5" fill="currentColor" />
    </svg>
  )
}

export function ApiHealthBadge({ state, compact = false, onRead: _onRead }: { state: ApiHealthState; compact?: boolean; onRead?: () => void }) {
  const { t } = useLanguage()
  ...
  return <div className={`api-health ${compact ? 'api-health-compact' : ''}${pulsing ? ' is-pulsing' : ''}`} ref={ref}>
    <button className={`api-health-badge api-health-${state.status}`} aria-label={label} title={label} onClick={() => setOpen(v => !v)}>
      <ApiSignalIcon status={state.status} size={compact ? 11 : 12} />
      {showLabel && <span className="api-health-label">{label}</span>}
    </button>
    {open && <div className="api-health-popover" role="dialog" aria-label={t('apiHealth.title')}>
      <div className="api-health-head">
        <strong>{t('apiHealth.title')}</strong>
        <span className="api-health-head-state">{label}</span>
        <button className="api-health-close" onClick={() => setOpen(false)} aria-label={t('common.close')}><IconX size={12} /></button>
      </div>
