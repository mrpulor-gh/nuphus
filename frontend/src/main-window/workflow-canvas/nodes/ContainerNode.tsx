...
export const ContainerNode = memo(function ContainerNode({ data, selected }: NodeProps<ContainerNodeFlow>) {
  ...
  return (
    <div className={classes} title="双击进入子层">
      ...
      <div className="wfc-node-head">
        ...
        {badge && badge.error > 0 && <span className="wfc-dot wfc-dot--error" title={`子层 ${badge.error} 个失败`} />}
        {badge && badge.error === 0 && badge.running > 0 && (
          <span className="wfc-dot wfc-dot--running" title={`子层 ${badge.running} 个运行中`} />
        )}
      </div>
      {node.containerSummary && <div className="wfc-node-summary" title={node.containerSummary}>{node.containerSummary}</div>}
          ...
          <ul className="wfc-children-preview-list">
            {childrenPreview.items.map((c, i) => (
              <li key={`${c.name}-${i}`} className="wfc-children-preview-item">
                <span className="wfc-children-preview-name" title={c.name}>{c.name}</span>
