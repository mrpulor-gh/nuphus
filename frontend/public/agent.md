# UI 原型设计：AI Agent 草图产出（beta）

UI 原型设计（Nuphus 画布能力）是一个浏览器端的 Material 3 Expressive 屏幕编辑画布。一份设计就是一个 JSON 文档。你（Agent）负责写出这份 JSON 并交回：用户在画布中打开、精修，再导出为编码工具的 prompt。此格式处于 beta，字段可能扩展但既有字段含义不变。

This format is in beta. Fields may be added; existing ones keep their meaning.

## What to deliver

**Reply with the JSON document itself in a code block**; the person saves it as a `.json` file and opens it with **Open project**. If the editor is reachable at a public origin, you may instead reply with a share link: encode the raw-deflated document into the URL fragment (`#docz=`) on that origin — see the snippets below. Either way, **do not verify, decode, or round-trip your output**: the app checks the document when it opens and tells the person what is wrong, so your checks add nothing.

To make a link when the editor has a public origin:
...
const base = process.env.EDITOR_ORIGIN || "";            // e.g. https://your-canvas.example
console.log(base + "/#docz=" + deflateRawSync(json).toString("base64url"));
```
...
base = os.environ.get("EDITOR_ORIGIN", "")               # e.g. https://your-canvas.example
print(base + "/#docz=" + base64.urlsafe_b64encode(raw).decode().rstrip("="))
```

The link is long (a few thousand characters for a few screens). That is expected; it carries the whole design and nothing is stored anywhere. Set `EDITOR_ORIGIN` to the origin where the person can open the design (the editor lives next to its guide).
