...
### Added
- 手机局域网 HTTPS 直连（18773）用户引导闭环：桌面「网络控制中心」局域网直连区分展示 HTTP 兼容地址与 HTTPS 地址（iOS 推荐），附首次安装局域网证书分步引导与 ca.crt 下载链接
- 配对引导页补充证书安装提示：中继 HTTPS 入口且缓存含桌面 HTTPS 局域网地址时，预提示下载安装桌面局域网证书
- 手机端「添加主屏图标」文案发布口径：中继 HTTPS 图标家里家外通用（在家自动直连局域网），局域网 http 页不再引导添加 http 图标当远程入口

### Changed
- 桌面 mobile_server 状态新增 lan_url_https 字段（与 /relay-hint 共用实际 HTTPS 端口事实源），二维码旁补充「统一入口 + 单个主屏图标」引导文案
- 双语文案（zh/en）同步扩展局域网地址/证书引导/图标引导键
