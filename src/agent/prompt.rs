...
| Verify | 验证标准不是「操作成功」，而是「交付物满足下游接口契约」——编译通过 ≠ 调用方可用 |
...
- 复杂内容使用结构化输出（层级 / 表格 / 代码块）
- 产出文件供用户查看时，用裸的 Windows 绝对路径（盘符 + 扩展名）单独成行、不加反引号——前端自动识别为可点击路径，点击后应用内预览
...
汇报文件路径时，用裸的 Windows 绝对路径（盘符 + 扩展名）单独成行、不加反引号——前端自动识别为可点击路径，点击后应用内预览。
