...
///
/// 解析顺序：
///   a. 内存权威：`runtime.llm_config.model == 生效model` 时返回其 provider
///      —— chat 实际用它路由，切换后与磁盘记录必然一致且最新；
///   b. 磁盘切换记录：`[last_model]` 查该 model；记录指向的 provider 段仍须
///      存在于 registry（段被删除时不信任脏记录，继续回落）；
///   c. registry 候选回落：`find_model_candidates` 唯一命中取之；多命中优先
///      段名含 "opencode" 的网关段（对齐 chat 组按模型族分流 opencode-go 的
///      transport 语义，见 provider.rs）；无命中 → 空串。
pub fn resolve_model_provider_core(
    ...
    if let Some((mem_model, mem_provider)) = memory {
        if mem_model == model && !mem_provider.is_empty() {
            return mem_provider.to_string();
        }
    }
    // b. [last_model] 磁盘切换记录
    if let Some(recorded) = load_last_model_provider(providers_path, model) {
        if !recorded.is_empty() && registry.providers.iter().any(|p| p.name == recorded) {
            return recorded;
        }
    }
    // c. registry 同 id 候选回落（多命中优先网关段）
    let candidates = registry.find_model_candidates(model);
    match candidates.len() {
        0 => String::new(),
        1 => candidates[0].0.name.clone(),
        _ => candidates
            .iter()
            .find(|(p, _)| p.name.contains("opencode"))
            .map(|(p, _)| p.name.clone())
            .unwrap_or_else(|| candidates[0].0.name.clone()),
    }
}
...
mod tests {
    ...
    fn provider_context_last_model_record_and_stale_fallback() {
        // GO 段无该 id（候选只有官方段）；[last_model] 记录 opencode-go → 记录赢。
    ...
    fn provider_context_candidates_fallback() {
        let (dir, registry) = dual_segment_registry(true);
        let path = dir.join("providers.toml");
        // 双段同 id 且无记录：多命中优先 opencode 网关段（对齐 chat 组族分流语义）
        assert_eq!(
            resolve_model_provider_core(None, &path, &registry, "deepseek-v4-flash"),
            "opencode-go"
        );
