//! Native AX window service. Numeric handles are process-local aliases to retained AX windows,
...
fn call(
    ...
    let sender = WORKER
        ...
        .as_ref()
        .map_err(|e| crate::NuphusError::Tool(e.clone()))?;
    ...
    sender
        ...
        .map_err(|_| crate::NuphusError::Tool("macOS window worker stopped".into()))?;
    let value = receive
        .recv_timeout(Duration::from_secs(15))
        .map_err(|_| crate::NuphusError::Tool("macOS window operation timed out".into()))?
        .map_err(crate::NuphusError::Tool)?;
    Ok(json!({"success": true, "result": value}))
}

pub fn windows_list() -> crate::Result<Value> {
    call(State::refresh)
}
pub fn window_info(handle: i32) -> crate::Result<Value> {
    call(move |state| {
        permission()?;
        state.info(handle)
    })
...
pub fn window_is_foreground(handle: i32) -> crate::Result<Value> {
    call(move |state| {
        state.info(handle)?;
        Ok(json!({"hwnd": handle, "foreground": state.foreground()? == handle}))
    })
}
pub fn window_activate(handle: i32) -> crate::Result<Value> {
    call(move |state| {
        permission()?;
...
pub fn window_move(handle: i32, x: i32, y: i32) -> crate::Result<Value> {
    set_pair(handle, "AXPosition", 1, x, y)
}
pub fn window_resize(handle: i32, width: i32, height: i32) -> crate::Result<Value> {
    if width <= 0 || height <= 0 {
        return Err(crate::NuphusError::Tool(
...
pub fn capture_window_id(handle: i32) -> crate::Result<u32> {
    let info = window_info(handle)?;
    let info = &info["result"];
    let bounds = &info["window"];
    let windows = xcap::Window::all().map_err(|e| crate::NuphusError::Tool(e.to_string()))?;
    ...
    if matches.len() != 1 {
        return Err(crate::NuphusError::Tool(
            "窗口截图无法唯一定位；请确认录屏权限并刷新窗口列表".into(),
        ));
    }
    matches[0]
        .id()
