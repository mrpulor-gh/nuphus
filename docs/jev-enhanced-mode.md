...
- 普通模式中的现有大模型只能通过结构化工具从当次候选中选择 `candidate_id`，不得优先生成坐标、任意快捷键或脚本。
...
4. **候选动作约束**：现有大模型也只能选择本地候选 ID，不得自由构造坐标、脚本、选择器或快捷键字符串。
...
- 默认关闭。
- 推荐开关副文案：`使用 Jev 参与动作选择、风险辅助与结果验证`。
...
UIA/AX/AT-SPI 原生 Pattern/Action 是首选执行方式。ExecutionGrant 允许视觉回退且目标仍可通过新观察稳定重定位时，代码可以从最新 rect 本地推导中心点；坐标永不发送给现有模型或 Jev，也不持久化为工作流主定位信息。
...
- 新桌面 Agent 路径中的主模型不可见自由坐标、`system_shell` 和脚本生成入口；旧工具只在兼容/显式降级路径可用。
...
- [ ] 普通模式中的现有大模型也只能选择本次候选或调用高层语义工具，不能自由生成坐标/脚本。
