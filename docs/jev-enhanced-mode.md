...
- 开启后，后续 Workflow 模式中每个新的可执行桌面决策都应优先进入 `desktop_agent_step`，使 Jev 实际参与本地候选的闭集选择；开关本身不会立刻控制电脑。Jev 明确转交主模型、服务不可用或语义树不适用时，仍可回退普通语义、视觉与鼠标工具。
...
关闭 Jev 时只替换 `Decide` 的实现，不移除 revision 检查、单步执行、重新观察、后置条件或 Stop/cancel。增强模式额外使用桌面语义专用的 100 步/连续无变化预算；普通模式继续受 WorkflowAgent 通用迭代上限和运行时死循环保护约束，不依赖 Jev。
...
### 12.7 2026-09-22 Windows 真实产品验收记录

本轮验收不是绕过 Nuphus 直接调用 Jev 或桌面驱动，而是从 Nuphus 可见 Workflow 界面新建会话、切换模式、输入自然语言需求，让应用内已配置的主模型与 Jev 共同生成、校验并运行工作流。隔离工作目录位于 E 盘，测试期间未读取或输出 API Key。

| 场景 | 模式与路由证据 | 结果 |
| --- | --- | --- |
| 记事本另存为 | 界面明确显示“增强模式，可用”，`aria-pressed=true`；生成阶段多次调用 `desktop_agent_step`，随后固化为 `desktop_semantic_action` | `workflow_validate` 通过；首次运行和保存后的“重新执行”均为 `RunCompleted: Success`；所有关键步骤 `on_error=abort`，保存按钮不再依赖 `skip`；目标文件内容逐字符一致 |
| 普通模式 UIA 对照 | 界面明确显示“增强模式，已关闭”，整个生成与运行阶段未调用 `desktop_agent_step`；仍使用 `desktop_semantic_observe` 和持久化 `desktop_semantic_action` | 工作流 `RunCompleted: Success`，证明 UIA/语义执行属于共享基础层，不依赖 Jev |
| 微信真实应用 | 界面显示“增强模式，可用”，Jev 参与候选选择；先尝试 UIA，Qt 控件的 Invoke 不产生预期切换时才降级到由本地最新元素边界推导的鼠标点击 | 先校验当前聊天对象为“张青华”，发送前执行重复守卫，仅发送一条带唯一标记的测试消息，再从聊天记录读回验证；最终工作流 14/14 Success |

本轮同时修复了 Windows UIA 的应用身份边界：同一可执行程序的主窗口与系统对话框可能有不同窗口类，应用 ID 现在优先基于本地进程映像生成；无法读取进程映像时才回退到框架与窗口类。执行前作用域不匹配仍会拒绝；只有成功 dispatch 后、原唯一 Invoke 目标消失且仍处于同一应用时，窗口切换才会被验证为 `Achieved`。这使记事本“保存”关闭对话框并回到主窗口成为可证明的成功，而不会把跨应用切换误判为成功。

真实验收也暴露两点后续优化方向：

- 微信 Qt UIA 覆盖不完整，基础层仍需改进非原生控件的可访问性适配；在此之前应保持“UIA 优先、视觉/鼠标降级”，不能把鼠标完全禁用。
- OCR/YOLO 首次加载输出大量 ONNX Runtime INFO 日志，影响诊断信噪比；应在后续独立改动中降低第三方推理日志级别，但不影响本轮功能正确性。
